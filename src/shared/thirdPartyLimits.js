...
function normalizeCustomEndpointPath(value) {
  ...
  try {
    const parsed = new URL(raw, 'https://quota-pulse.invalid');
    if (
      parsed.origin !== 'https://quota-pulse.invalid'
      || parsed.search
      || parsed.hash
    ) return '';
