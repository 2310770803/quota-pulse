...
const LIMIT_PROVIDER_IDS = ['codex', 'opencode', 'cursor'];
...
const QUOTA_PULSE_USER_AGENT = `quota-pulse/${appVersion()} (+https://github.com/Javis603/quota-pulse)`;
  ...
  try {
    const response = await fetchFn(url, {
      ...
      headers: {
        ...
        'user-agent': QUOTA_PULSE_USER_AGENT
      },
      body: body.toString(),
...
function claudeCommandCandidates(env = process.env, platform = process.platform) {
  if (env.QUOTA_PULSE_CLAUDE_COMMAND) return [env.QUOTA_PULSE_CLAUDE_COMMAND];
  const candidates = [];
...
cmd = os.environ.get("QUOTA_PULSE_CLAUDE_COMMAND_PATH", "claude")
cwd = os.environ.get("QUOTA_PULSE_CLAUDE_PROBE_DIR") or os.getcwd()
timeout = float(os.environ.get("QUOTA_PULSE_CLAUDE_CLI_TIMEOUT", "35"))
slash_command = os.environ.get("QUOTA_PULSE_CLAUDE_SLASH_COMMAND", "/usage")
exit_marker = os.environ.get("QUOTA_PULSE_CLAUDE_EXIT_MARKER_REGEX", "currentsession.*?[0-9]{1,3}(?:\\\\.[0-9]+)?%")
...
async function runClaudePtyProbe(slashCommand, exitMarkerRegex, deps = {}) {
  ...
  const probeDir = deps.claudeProbeDir || path.join(os.tmpdir(), 'quota-pulse-claude-probe');
  fs.mkdirSync(probeDir, { recursive: true });
  const runEnv = {
    ...env,
    QUOTA_PULSE_CLAUDE_COMMAND_PATH: command,
    QUOTA_PULSE_CLAUDE_PROBE_DIR: probeDir,
    QUOTA_PULSE_CLAUDE_CLI_TIMEOUT: String(deps.claudeCliTimeoutSeconds || 35),
    QUOTA_PULSE_CLAUDE_SLASH_COMMAND: slashCommand,
    QUOTA_PULSE_CLAUDE_EXIT_MARKER_REGEX: exitMarkerRegex || ''
  };
...
function codexCommandCandidates(env = process.env, platform = process.platform, deps = {}) {
  if (env.QUOTA_PULSE_CODEX_COMMAND) return [env.QUOTA_PULSE_CODEX_COMMAND];
  const pathApi = pathApiForPlatform(platform);
  const candidates = [];
  if (platform === 'darwin') {
    candidates.push(
...
async function readCodexRpcWithCommand(command, deps = {}) {
  ...
  try {
    ...
    await rpc.send('initialize', {
          clientInfo: { name: 'quota-pulse', title: 'Quota Pulse', version: appVersion() }
    });
...
async function fetchOpenCodeLimits(options = {}, deps = {}) {
  ...
  const envCookie = (deps.env || process.env).QUOTA_PULSE_OPENCODE_COOKIE || '';
