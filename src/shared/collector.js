...
function cachedAnchorPreview(options = {}) {
  const anchor = options.anchor;
  if (!anchor?.today || !anchor?.month || !anchor?.allTime) return null;
  const wsl = options.wslAnchor;
  const today = wsl?.today ? mergePeriods(anchor.today, wsl.today) : anchor.today;
  const month = wsl?.month ? mergePeriods(anchor.month, wsl.month) : anchor.month;
  const allTime = wsl?.allTime ? mergePeriods(anchor.allTime, wsl.allTime) : anchor.allTime;
  const deviceOsInfo = normalizeOsInfo(options.osInfo);
  return {
    ...
    trackedClients: String(options.clients || '').split(',').map((client) => client.trim()).filter(Boolean),
    today,
    month,
    allTime,
    clientStatus: deriveClientStatus(options.clients, allTime),
    ...(options.wslStatus ? { wslStatus: options.wslStatus } : {}),
    projectsEnabled: options.projectsEnabled !== false
  };
}
...
function startCollector(options) {
        ...
        if (saved.configFingerprint === fp) {
          ...
          if (saved.fullScanAt) {
            ...
            if (Number.isFinite(parsed) && parsed <= Date.now()) {
              ...
              anchorUpdatedAt = saved.fullScanAt;
            }
          }
        }
      }
  ...
  const startupPreview = anchorUpdatedAt ? cachedAnchorPreview({
    anchor,
    wslAnchor,
  ...
  if (startupPreview && typeof onPreview === 'function') {
    // Publish the last complete same-day snapshot before spawning tokscale. A
    // fresh scan still starts immediately below and replaces this cached view.
    queueMicrotask(() => {
      if (!stopped) onPreview(startupPreview, 'cache');
    });
  }
  setupWatchers();
  loop();
