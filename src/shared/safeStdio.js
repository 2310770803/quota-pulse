...
function installSafeStdout() {
  if (process.stdout._quotaPulseEpipeHandled) return;
  process.stdout._quotaPulseEpipeHandled = true;
  process.stdout.on('error', (err) => {
    if (!err || err.code !== 'EPIPE') throw err;
  });
  // stderr mirrors stdout; install a handler there too for symmetry.
  if (!process.stderr._quotaPulseEpipeHandled) {
    process.stderr._quotaPulseEpipeHandled = true;
