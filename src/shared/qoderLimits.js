...
function qoderCookie(env = process.env, options = {}) {
  ...
  for (const name of ['QODER_COOKIE', 'QUOTA_PULSE_QODER_COOKIE']) {
...
function qoderSite(options = {}, env = process.env) {
  const value = String(options.qoderSite || env.QODER_SITE || env.QUOTA_PULSE_QODER_SITE || '').trim().toLowerCase();
