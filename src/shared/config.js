...
function applyLegacyEnvAliases(env = process.env) {
  for (const [key, value] of Object.entries(env || {})) {
    if (!key.startsWith('TOKEN_MONITOR_')) continue;
    const canonicalKey = `QUOTA_PULSE_${key.slice('TOKEN_MONITOR_'.length)}`;
    if (env[canonicalKey] === undefined || env[canonicalKey] === '') {
...
function sharedDataDir(options = {}) {
  const env = applyLegacyEnvAliases(options.env || process.env);
  if (env.QUOTA_PULSE_SHARED_DIR) return env.QUOTA_PULSE_SHARED_DIR;
  const platform = options.platform || process.platform;
  const homeDir = options.homeDir || os.homedir();
  const productName = 'Quota Pulse';
