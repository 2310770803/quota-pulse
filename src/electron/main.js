...
const { sendFeishuMessage } = require('../shared/feishuPush');
const { sendEmailMessage, emailConfigured } = require('../shared/mailPush');
const { buildQuotaReport } = require('../shared/pushReport');
const { createPushScheduler } = require('../shared/pushScheduler');
...
const APP_NAME = 'Quota Pulse';
const APP_ICON_PATH = app.isPackaged
  ? path.join(process.resourcesPath, 'assets', 'app-icon.png')
  : path.join(__dirname, '..', '..', '..', 'assets', 'app-icon.png');

const DEFAULT_WINDOW = { width: 340, height: 520 };
...
const DEFAULT_VIEW_LIST = ['home', 'limits', 'session', 'tool', 'model'].map((id) => ({ id }));
const DEFAULT_HOME_MODULE_LIST = ['limits', 'tool', 'model'].map((id) => ({ id }));
const TRAY_OPEN_VIEW_IDS = new Set(['home', 'session', 'limits']);
...
// Preserve the existing settings and credentials directory while the visible
// product name changes. This avoids a "fresh install" experience after rebrand.
app.setPath('userData', path.join(app.getPath('appData'), 'AIQuotaPulse'));
if (process.platform === 'win32') app.setAppUserModelId('com.quotapulse.app');
...
function defaultSettings() {
  const envHubUrl = process.env.QUOTA_PULSE_HUB_URL || '';
  const windowBehavior = process.env.QUOTA_PULSE_ALWAYS_ON_TOP === '0' ? 'normal' : 'floating';
  return {
    hubMode: envHubUrl ? 'client' : 'local',
    hubUrl: envHubUrl,
    hubHostPort: Math.max(1, Math.min(65535, Number(process.env.QUOTA_PULSE_PORT) || HUB_DEFAULT_PORT)),
    // Default to QUOTA_PULSE_SECRET so agents that already trust this env
    ...
    hubHostSecret: process.env.QUOTA_PULSE_SECRET || '',
    secret: process.env.QUOTA_PULSE_SECRET || '',
    windowBehavior,
    alwaysOnTop: windowBehavior === 'floating',
    refreshMs: Number(process.env.QUOTA_PULSE_WIDGET_REFRESH_MS || 15000),
    glassOpacity: 68,
    glassBlur: 32,
    systemGlass: true,
    ...
    deviceId: process.env.QUOTA_PULSE_DEVICE_ID || defaultDeviceId(),
    lastPostedDeviceId: '',
    clients: clientsCsvForSetting(process.env.QUOTA_PULSE_CLIENTS),
    clientDisplayOrder: '',
    hiddenClients: '',
    pinnedClients: '',
    viewDisplayOrder: '',
    ...
    projectsEnabled: parseBoolean(process.env.QUOTA_PULSE_PROJECTS_ENABLED, false),
    historyEnabled: true,
    historyIntervalMs: normalizeHistoryIntervalMs(process.env.QUOTA_PULSE_HISTORY_INTERVAL_MS),
    sessionUsageArchiveEnabled: parseBoolean(process.env.QUOTA_PULSE_SESSION_USAGE_ARCHIVE_ENABLED, true),
    wslScanEnabled: false,
    collectionMode: 'live',
    collectionIntervalMs: 5 * 60 * 1000,
    syncUploadIntervalMs: normalizeSyncUploadIntervalMs(process.env.QUOTA_PULSE_SYNC_UPLOAD_INTERVAL_MS),
    serviceProviderDisplayOrder: '',
    hiddenServiceProviders: '',
    serviceStatusRefreshMs: 60000,
    archivedClientUsage: { version: 1, clients: {} },
    allTimeSince: process.env.QUOTA_PULSE_ALL_TIME_SINCE || '2024-01-01',
    customModelPricing: [],
    limitsEnabled: parseBoolean(process.env.QUOTA_PULSE_LIMITS_ENABLED, true),
    limitProviders: parseLimitProviders(process.env.QUOTA_PULSE_LIMIT_PROVIDERS).join(','),
    ...
    limitsRefreshMs: normalizeLimitsRefreshMs(process.env.QUOTA_PULSE_LIMITS_REFRESH_MS),
    showLimitSource: parseBoolean(process.env.QUOTA_PULSE_SHOW_LIMIT_SOURCE, false),
    maskLimitAccountEmails: false,
    showLimitUsed: parseBoolean(process.env.QUOTA_PULSE_SHOW_LIMIT_USED, false),
    windowBounds: null,
    ...
    currency: normalizeCurrency(process.env.QUOTA_PULSE_CURRENCY || 'USD'),
    currencyRates: {},
    // Push notifications (Feishu webhook + SMTP email)
    pushFeishuEnabled: false,
    pushFeishuWebhook: '',
    pushFeishuSignKey: '',
    pushEmailEnabled: false,
    pushEmailSender: '',
    pushEmailAuthCode: '',
    pushEmailRecipients: '',
    pushEmailSmtpServer: 'smtp.qq.com',
    pushEmailSmtpPort: 465,
    pushAlertLowEnabled: true,
    pushAlertLowThreshold: 10,
    pushAlertWarningEnabled: true,
    pushAlertWarningThreshold: 30,
    pushAlertRestoredEnabled: true,
    pushAlertRestoredThreshold: 100,
    pushReportIntervalEnabled: true,
    pushReportIntervalHours: 5,
    pushReportDailyEnabled: false,
    pushReportDailyTime: '22:00',
    startAtLogin: false,
    ...
    zaiApiKey: '',
    zaiApiRegion: normalizeZaiApiRegion(process.env.QUOTA_PULSE_ZAI_API_REGION || process.env.ZAI_API_REGION || process.env.Z_AI_API_HOST || 'global'),
    zaiTeamApiKey: '',
...
function defaultLimitProviders() {
  return parseLimitProviders(process.env.QUOTA_PULSE_LIMIT_PROVIDERS).join(',');
}
...
function normalizeHubMode(_value, _fallback = 'local') {
  // Quota Pulse is local-only: multi-device sync was removed.
  return 'local';
}
...
function reportCredentialStorageError(context, error) {
  ...
  try {
    dialog.showErrorBox(
      ...
      `Quota Pulse could not safely access credentials.json (${context}). The save was stopped and previous data was restored where possible. Check the file's JSON and permissions, then restart the app.\n\n${detail}`
    );
  } catch (_) {}
}
...
function readSettings() {
  ...
  try {
    ...
    if (Number(saved.windowLayoutVersion || 0) < WINDOW_LAYOUT_VERSION) {
      const savedBounds = saved.windowBounds && typeof saved.windowBounds === 'object'
        ? saved.windowBounds
        : null;
      merged.windowBounds = savedBounds
        ? { ...savedBounds, width: DEFAULT_WINDOW.width, height: DEFAULT_WINDOW.height }
        : null;
      merged.zoomFactor = Math.min(clampZoom(saved.zoomFactor), DEFAULT_ZOOM_FACTOR);
      merged.windowLayoutVersion = WINDOW_LAYOUT_VERSION;
...
function getDefaultTrayIcon() {
  if (!defaultTrayIcon) defaultTrayIcon = buildTrayIcon({ iconPath: APP_ICON_PATH });
...
async function startEmbeddedHub() {
  // Quota Pulse is local-only: embedded hub was removed.
  return null;
}
...
function updateTrayDisplay() {
  ...
  tray.setToolTip(`Quota Pulse - ${tip}`);
...
function stopAll() {
  ...
  if (pushScheduler) { try { pushScheduler.stop(); } catch (_) {} }
  if (tray && !tray.isDestroyed()) tray.destroy();
  tray = null;
}
...
let pushLastResult = null;

// Collapse multi-account duplicates of the same provider into one entry,
// keeping the worst (lowest remaining) account so alerts fire on the weakest
// account first.
function pushProviderEntries() {
  const providers = Array.isArray(latestStats?.limits?.providers) ? latestStats.limits.providers : [];
  ...
  for (const entry of providers) {
    ...
    const remainingOf = (e) => {
      const win = (Array.isArray(e?.windows) ? e.windows : []).find((w) => w && w.remainingPercent !== null && w.remainingPercent !== undefined);
      return win ? Number(win.remainingPercent) : 101;
    };
    if (remainingOf(entry) < remainingOf(existing)) byProvider.set(id, entry);
  }
  return [...byProvider.values()];
}

function currentPushServices() {
  return buildQuotaReport(pushProviderEntries()).services;
}

function pushConfigFromSettings() {
  return {
    alertLowEnabled: settings?.pushAlertLowEnabled,
    alertLowThreshold: settings?.pushAlertLowThreshold,
    alertWarningEnabled: settings?.pushAlertWarningEnabled,
    alertWarningThreshold: settings?.pushAlertWarningThreshold,
    alertRestoredEnabled: settings?.pushAlertRestoredEnabled,
    alertRestoredThreshold: settings?.pushAlertRestoredThreshold,
    reportIntervalEnabled: settings?.pushReportIntervalEnabled,
    reportIntervalHours: settings?.pushReportIntervalHours,
    reportDailyEnabled: settings?.pushReportDailyEnabled,
    reportDailyTime: settings?.pushReportDailyTime
  };
}

async function sendPushReport(kind) {
  const { text } = buildQuotaReport(pushProviderEntries());
  const channels = [];
  const errors = [];
  if (settings?.pushFeishuEnabled) {
    if (!settings.pushFeishuWebhook) {
      errors.push('飞书：配置不完整');
    } else {
      try {
        await sendFeishuMessage({ webhookUrl: settings.pushFeishuWebhook, signKey: settings.pushFeishuSignKey || '', text });
        channels.push('feishu');
      } catch (error) {
        errors.push(`飞书：${error.message}`);
      }
    }
  }
  if (settings?.pushEmailEnabled) {
    const mailConfig = {
      sender: settings.pushEmailSender,
      authCode: settings.pushEmailAuthCode,
      recipients: settings.pushEmailRecipients,
      smtpServer: settings.pushEmailSmtpServer,
      smtpPort: settings.pushEmailSmtpPort
    };
    if (!emailConfigured(mailConfig)) {
      errors.push('邮件：配置不完整');
    } else {
      try {
        await sendEmailMessage({ ...mailConfig, text });
        channels.push('email');
      } catch (error) {
        errors.push(`邮件：${error.message}`);
      }
    }
  }
  const ok = channels.length > 0;
  pushLastResult = { at: new Date().toISOString(), kind, ok, channels, errors };
  if (mainWindow && !mainWindow.isDestroyed()) {
    try { mainWindow.webContents.send('push:result', pushLastResult); } catch (_) {}
  }
  if (!ok) console.log(`[push] report not sent (${kind}): ${errors.join('；') || 'no channel enabled'}`);
  else console.log(`[push] report sent (${kind}): ${channels.join(', ')}`);
  return { ok, channels, errors };
}

async function testPushChannel(channel) {
    const text = `Quota Pulse 推送测试\n检测时间：${new Date().toLocaleString('zh-CN', { hour12: false })}`;
  if (channel === 'feishu') {
    if (!settings?.pushFeishuWebhook) return { ok: false, error: '飞书：请先填写 Webhook 地址' };
    try {
      await sendFeishuMessage({ webhookUrl: settings.pushFeishuWebhook, signKey: settings.pushFeishuSignKey || '', text });
      return { ok: true };
    } catch (error) {
  ...
  if (channel === 'email') {
    ...
    if (!emailConfigured(mailConfig)) return { ok: false, error: '邮件：配置不完整' };
    try {
      await sendEmailMessage({ ...mailConfig, subject: 'Quota Pulse 推送测试', text });
      return { ok: true };
    } catch (error) {
      return { ok: false, error: error.message };
    }
...
function startPushScheduler() {
  if (pushScheduler) return;
  pushScheduler = createPushScheduler({
    statePath: path.join(app.getPath('userData'), 'push-state.json'),
    getConfig: pushConfigFromSettings,
    getServices: async () => currentPushServices(),
    sendReport: sendPushReport,
    logger: console
  });
  pushScheduler.start();
}
...
function startAppUpdateBackgroundChecks() {
  // Quota Pulse: in-app auto update removed; no background checks.
}
...
async function downloadAndPrepareAppUpdate() {
  // Quota Pulse: in-app auto update removed.
  return deriveAppUpdateState();
}
...
function isAllowedExternalUrl(value) {
  ...
  if (parsed.hostname === 'github.com' && parsed.pathname.startsWith('/Javis603/quota-pulse')) return true;
...
function scheduleStartupServices(target) {
  let started = false;
  let fallbackTimer = null;
  const start = () => {
    if (started || quitRequested) return;
    started = true;
    if (fallbackTimer) clearTimeout(fallbackTimer);
    // Let Chromium finish parsing, layout, and the first paint before starting
    // collectors and optional disk/network work. This avoids competing with the
    // renderer during the most visible part of a cold launch.
    setTimeout(() => {
      if (quitRequested) return;
  ...
  // A renderer load failure must not leave background services permanently off.
  fallbackTimer = setTimeout(start, 1200);
}
...
app.whenReady().then(() => {
  ...
  scheduleStartupServices(mainWindow);
  ...
  ipcMain.handle('settings:update', (_event, patch) => {
    ...
    settings = normalizeWindowBehaviorSettings({
      ...
      pushFeishuEnabled: parseBoolean(patch.pushFeishuEnabled ?? settings.pushFeishuEnabled, false),
      pushFeishuWebhook: patch.pushFeishuWebhook !== undefined ? String(patch.pushFeishuWebhook || '').trim() : (settings.pushFeishuWebhook || ''),
      pushFeishuSignKey: patch.pushFeishuSignKey !== undefined ? String(patch.pushFeishuSignKey || '').trim() : (settings.pushFeishuSignKey || ''),
      pushEmailEnabled: parseBoolean(patch.pushEmailEnabled ?? settings.pushEmailEnabled, false),
      pushEmailSender: patch.pushEmailSender !== undefined ? String(patch.pushEmailSender || '').trim() : (settings.pushEmailSender || ''),
      pushEmailAuthCode: patch.pushEmailAuthCode !== undefined ? String(patch.pushEmailAuthCode || '').trim() : (settings.pushEmailAuthCode || ''),
      pushEmailRecipients: patch.pushEmailRecipients !== undefined ? String(patch.pushEmailRecipients || '').trim() : (settings.pushEmailRecipients || ''),
      pushEmailSmtpServer: patch.pushEmailSmtpServer !== undefined ? (String(patch.pushEmailSmtpServer || '').trim() || 'smtp.qq.com') : (settings.pushEmailSmtpServer || 'smtp.qq.com'),
      pushEmailSmtpPort: patch.pushEmailSmtpPort !== undefined ? (Number(patch.pushEmailSmtpPort) || 465) : (Number(settings.pushEmailSmtpPort) || 465),
      pushAlertLowEnabled: parseBoolean(patch.pushAlertLowEnabled ?? settings.pushAlertLowEnabled, true),
      pushAlertLowThreshold: Math.max(1, Math.min(99, Number(patch.pushAlertLowThreshold ?? settings.pushAlertLowThreshold) || 10)),
      pushAlertWarningEnabled: parseBoolean(patch.pushAlertWarningEnabled ?? settings.pushAlertWarningEnabled, true),
      pushAlertWarningThreshold: Math.max(1, Math.min(99, Number(patch.pushAlertWarningThreshold ?? settings.pushAlertWarningThreshold) || 30)),
      pushAlertRestoredEnabled: parseBoolean(patch.pushAlertRestoredEnabled ?? settings.pushAlertRestoredEnabled, true),
      pushAlertRestoredThreshold: Math.max(1, Math.min(100, Number(patch.pushAlertRestoredThreshold ?? settings.pushAlertRestoredThreshold) || 100)),
      pushReportIntervalEnabled: parseBoolean(patch.pushReportIntervalEnabled ?? settings.pushReportIntervalEnabled, true),
      pushReportIntervalHours: Math.max(1, Math.min(168, Number(patch.pushReportIntervalHours ?? settings.pushReportIntervalHours) || 5)),
      pushReportDailyEnabled: parseBoolean(patch.pushReportDailyEnabled ?? settings.pushReportDailyEnabled, false),
      pushReportDailyTime: /^\d{1,2}:\d{2}$/.test(String(patch.pushReportDailyTime ?? settings.pushReportDailyTime ?? '')) ? String(patch.pushReportDailyTime ?? settings.pushReportDailyTime) : '22:00',
  ...
  ipcMain.handle('push:test', (_event, channel) => testPushChannel(channel));
  ipcMain.handle('push:sendNow', () => sendPushReport('regular_report'));
  ...
  ipcMain.handle('app:getInfo', () => ({
    ...
    arch: process.arch,
    appIconDataUrl: nativeImage
      .createFromPath(APP_ICON_PATH)
      .resize({ width: 88, height: 88, quality: 'best' })
      .toDataURL(),
  ...
  ipcMain.handle('opencode:status', async () => {
    ...
    const envCookie = process.env.QUOTA_PULSE_OPENCODE_COOKIE || '';
  ...
  ipcMain.handle('opencode:getProfiles', async () => {
    const profiles = settings.opencodeProfiles || {};
    const hasEnvVar = Boolean(process.env.QUOTA_PULSE_OPENCODE_COOKIE);
  ...
  ipcMain.handle('opencode:refresh', async () => {
    if (!deviceRuntimeHandle) return { ok: false, error: 'runtimeNotReady' };
    try {
      await deviceRuntimeHandle.refreshLimits({ provider: 'opencode' }, 'startup-refresh');
      return { ok: true };
    } catch (error) {
