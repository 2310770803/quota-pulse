...
function createAccentApi(koffi) {
  ...
  const ACCENT_POLICY = koffi.struct('QUOTA_PULSE_ACCENT_POLICY', {
    AccentState: 'int32_t',
    AccentFlags: 'int32_t',
    GradientColor: 'uint32_t',
    AnimationId: 'int32_t'
  });
  koffi.struct('QUOTA_PULSE_WINDOWCOMPOSITIONATTRIBDATA', {
    Attrib: 'uint32_t',
    pvData: 'void *',
    cbData: 'size_t'
  });
  koffi.struct('QUOTA_PULSE_DWM_BLURBEHIND', {
    dwFlags: 'uint32_t',
    fEnable: 'int32_t',
    hRgnBlur: 'void *',
    fTransitionOnMaximized: 'int32_t'
  });
  koffi.struct('QUOTA_PULSE_MARGINS', {
    cxLeftWidth: 'int32_t',
  ...
  const SetWindowCompositionAttribute = user32.func(
    'bool SetWindowCompositionAttribute(uintptr_t hwnd, const QUOTA_PULSE_WINDOWCOMPOSITIONATTRIBDATA *data)'
  );
  const DwmEnableBlurBehindWindow = dwmapi.func(
    'int DwmEnableBlurBehindWindow(uintptr_t hwnd, const QUOTA_PULSE_DWM_BLURBEHIND *blurBehind)'
  );
  const DwmExtendFrameIntoClientArea = dwmapi.func(
    'int DwmExtendFrameIntoClientArea(uintptr_t hwnd, const QUOTA_PULSE_MARGINS *margins)'
  );
    ...
    apply(hwnd, argb) {
      ...
      try {
        ...
        const data = {
          ...
          pvData: koffi.as(accent, 'QUOTA_PULSE_ACCENT_POLICY *'),
