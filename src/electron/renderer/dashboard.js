...
async function refresh() {
  ...
  try {
    ...
    state.history = await window.quotaPulse.getDashboardHistory();
    render();
  } catch (error) {
    state.motion = 'none';
...
async function boot() {
  let settings = {};
  try { settings = await window.quotaPulse.getSettings(); } catch (_) {}
  state.locale = i18n.resolveLocale(settings.language, navigator.languages);
  state.currency = settings.currency || 'USD';
  if (settings.currencyRatesEffective && window.QuotaPulseCurrency?.configureRates) {
    window.QuotaPulseCurrency.configureRates(settings.currencyRatesEffective);
  }
  ...
  render();
  await refresh();
  window.quotaPulse.dashboard.ready();
}
...
window.quotaPulse.onSettingsPush?.((next) => {
  ...
  if (next.currencyRatesEffective && window.QuotaPulseCurrency?.configureRates) {
    window.QuotaPulseCurrency.configureRates(next.currencyRatesEffective);
...
window.quotaPulse.onDashboardHistoryChanged?.(() => { void refresh(); });
...
els.heatmapMetricBtns.forEach((b) => b.addEventListener('click', () => {
  ...
  state.motion = 'none';
  render();
  window.quotaPulse.updateSettings({ heatmapMetric: state.heatmapMetric });
}));
els.themeToggle.addEventListener('click', () => { state.flat = !state.flat; els.body.classList.toggle('flat', state.flat); window.quotaPulse.updateSettings({ dashboardFlat: state.flat }); });
els.refreshBtn.addEventListener('click', refresh);
els.minBtn.addEventListener('click', () => window.quotaPulse.dashboard.minimize());
els.closeBtn.addEventListener('click', () => window.quotaPulse.dashboard.close());
