...
const { clientColors, fallbackModelColors, modelVendorFor, modelColor } = window.QuotaPulseUsageCharts;
const motionPreferenceApi = window.QuotaPulseMotionPreference;
const windowsGlassApi = window.QuotaPulseWindowsGlass;
const glassRenderingApi = window.QuotaPulseGlassRendering;
const wslStatusPresentationApi = window.QuotaPulseWslStatusPresentation;
...
const CORE_CLIENT_IDS = new Set(['codex', 'opencode', 'cursor']);
const VISIBLE_CLIENTS = KNOWN_CLIENTS.filter(({ id }) => CORE_CLIENT_IDS.has(id));
...
const CORE_LIMIT_PROVIDER_IDS = new Set(['codex', 'opencode', 'cursor']);
const VISIBLE_LIMIT_PROVIDERS = LIMIT_PROVIDERS.filter(({ id }) => CORE_LIMIT_PROVIDER_IDS.has(id));
...
const limitProviderOrderApi = window.QuotaPulseLimitProviderOrder;
const limitProviderPresentationApi = window.QuotaPulseLimitProviderPresentation;
const appUpdatePresentationApi = window.QuotaPulseAppUpdatePresentation;
const accountIdentityApi = window.QuotaPulseAccountIdentity;
const clientStatusPresentationApi = window.QuotaPulseClientStatusPresentation;
const serviceStatusPresentationApi = window.QuotaPulseServiceStatusPresentation;
const clientDisplayPreferencesApi = window.QuotaPulseClientDisplayPreferences;
const customPricingFormApi = window.QuotaPulseCustomPricingForm;
const viewDisplayPreferencesApi = window.QuotaPulseViewDisplayPreferences;
const preferenceDragSortApi = window.QuotaPulsePreferenceDragSort;
const homeOverviewApi = window.QuotaPulseHomeOverview;
const homeModulePreferencesApi = window.QuotaPulseHomeModulePreferences;
const { limitFillPercent, limitModeSuffix } = window.QuotaPulseLimitDisplayMode;
const i18n = window.QuotaPulseI18n;
const currencyApi = window.QuotaPulseCurrency;
const trayLayoutApi = window.QuotaPulseTrayLayout;
const sessionRowsApi = window.QuotaPulseSessionRows;
const breakdownRenderPolicyApi = window.QuotaPulseBreakdownRenderPolicy;
const {
...
const deviceBreakdownApi = window.QuotaPulseDeviceBreakdown;
const projectRowsApi = window.QuotaPulseProjectRows;
const sessionDetailApi = window.QuotaPulseSessionDetail;
const windowShortcutApi = window.QuotaPulseWindowShortcut;
...
const viewBreakdownValues = new Set(VIEW_DISPLAY_OPTIONS.map(({ id }) => id));
...
const QUOTA_PULSE_REPOSITORY_URL = 'https://github.com/Javis603/quota-pulse';
const QUOTA_PULSE_ISSUES_URL = `${QUOTA_PULSE_REPOSITORY_URL}/issues/new/choose`;
const QUOTA_PULSE_WSL_SQLITE_GUIDE_URL = `${QUOTA_PULSE_REPOSITORY_URL}/blob/main/docs/wsl-sqlite-setup.md`;
const serviceStatusProviderPreferencesApi = window.QuotaPulseServiceStatusProviderPreferences;
const SETTINGS_SECTION_IDS = ['general', 'main', 'window', 'appearance', 'tools', 'limits', 'accounts', 'push'];
const REFRESH_BUTTON_FEEDBACK_MS = 700;
const CODEX_PENDING_ACTIVE_GRACE_MS = 30000;
const initialFloatingBubble = window.__QUOTA_PULSE_INITIAL_FLOATING_BUBBLE__ || { collapsed: false, side: null };
const initialViewState = window.__QUOTA_PULSE_INITIAL_VIEW_STATE__ || {};
...
function applySettingsSectionDom(id, open) {
  ...
  const navigationItem = document.querySelector(`[data-settings-target="${id}"]`);
  ...
  navigationItem?.classList.toggle('is-active', open);
  if (open) navigationItem?.setAttribute('aria-current', 'page');
  else navigationItem?.removeAttribute('aria-current');
  if (open) {
    const title = navigationItem?.querySelector(':scope > span:nth-child(2)')?.textContent?.trim();
    const summary = els[`${id}SettingsSummary`]?.textContent?.trim();
    const workspaceTitle = document.getElementById('settingsWorkspaceTitle');
    const workspaceSummary = document.getElementById('settingsWorkspaceSummary');
    if (workspaceTitle && title) workspaceTitle.textContent = title;
    if (workspaceSummary) workspaceSummary.textContent = summary || '';
  }
}
...
function setupSettingsSections() {
  ...
  for (const navigationItem of document.querySelectorAll('[data-settings-target]')) {
    navigationItem.addEventListener('click', () => {
      const section = navigationItem.dataset.settingsTarget;
      cancelSettingsScrollAnchor();
      setSettingsSectionExpanded(section, true);
      els.settingsPanel?.scrollTo({ top: 0, behavior: 'auto' });
      document.querySelector('.settings-workspace')?.scrollTo({ top: 0, behavior: 'auto' });
    });
  }
...
function orderAccountProviderGroups() {
  ...
  if (!container) return;
  for (const provider of VISIBLE_LIMIT_PROVIDERS) {
...
function settingsSectionSummary(section) {
  ...
  if (section === 'push') {
    const channels = [];
    if (state.settings.pushFeishuEnabled) channels.push(t('settings.push.feishu.title'));
    if (state.settings.pushEmailEnabled) channels.push(t('settings.push.email.title'));
    if (channels.length === 0) return t('settings.push.summaryOff');
    return t('settings.push.summaryOn', { channels: channels.length });
  }
  if (section === 'tools') {
    return t('settings.summary.tools', {
      tracked: [...enabledClientSet()].filter((id) => CORE_CLIENT_IDS.has(id)).length,
      visible: VISIBLE_CLIENTS.length - [...hiddenClientSet()].filter((id) => CORE_CLIENT_IDS.has(id)).length,
  ...
  if (section === 'accounts') {
    ...
    return t('settings.summary.accounts', {
      linked: (codexLinked ? 1 : 0) + (cursorLinked ? 1 : 0) + (opencodeCount > 0 ? 1 : 0),
      total: VISIBLE_LIMIT_PROVIDERS.length
    });
  }
  if (section === 'limits') {
...
function renderSettingsSummaries() {
  for (const section of SETTINGS_SECTION_IDS) {
    ...
    const summary = settingsSectionSummary(section);
    if (el) el.textContent = summary;
    const navigationSummary = document.getElementById(`${section}SettingsNavSummary`);
    if (navigationSummary) navigationSummary.textContent = summary;
    if (state.settingsSections[section]) {
      const workspaceSummary = document.getElementById('settingsWorkspaceSummary');
      if (workspaceSummary) workspaceSummary.textContent = summary;
    }
  }
...
function formatReset(value) {
  ...
  if (diffMs === 0) return '立即重置';
  return `${formatDuration(diffMs)}后重置`;
}
...
function quotaHealthWindows() {
  return homeLimitRows().flatMap((row) => (
    Array.isArray(row?.windows)
      ? row.windows.map((window) => ({
        provider: { provider: row.providerId, label: row.name },
        window
      }))
      : []
  )).filter(({ window }) => (
...
function drawQuotaHealthRing(percent, stateName = 'neutral') {
  const canvas = els.quotaHealthRing;
  const ctx = canvas?.getContext?.('2d');
  if (!canvas || !ctx) return;
  const bounds = canvas.getBoundingClientRect();
  const size = Math.max(1, Math.min(bounds.width || 54, bounds.height || 54));
  const pixelRatio = Math.max(2, Math.min(4, Number(window.devicePixelRatio) || 1));
  ...
  const center = size / 2;
  const lineWidth = Math.max(3.5, size * 0.078);
  const radius = center - (lineWidth / 2) - 0.75;
  const styles = getComputedStyle(document.documentElement);
  const accent = styles.getPropertyValue('--qp-accent').trim() || '#5d63f1';
  const healthy = styles.getPropertyValue('--qp-success').trim() || '#209b71';
  const warning = styles.getPropertyValue('--qp-warning').trim() || '#eea44f';
  const critical = styles.getPropertyValue('--qp-critical').trim() || '#eb6a78';
  const track = styles.getPropertyValue('--qp-ring-track').trim() || '#e7e9f8';
  const color = stateName === 'critical'
    ? critical
    : (stateName === 'warning' ? warning : (stateName === 'healthy' ? healthy : accent));
...
function renderQuotaHealthSummary() {
  if (!els.quotaHealthSummary) return;
  const candidates = quotaHealthWindows();
  if (candidates.length === 0) {
    els.quotaHealthSummary.dataset.state = 'neutral';
    els.quotaHealthPercent.textContent = '—';
    els.quotaHealthTitle.textContent = '正在等待额度数据';
    els.quotaHealthReset.textContent = '刷新后检查额度';
    els.quotaHealthBadge.textContent = '实时';
    drawQuotaHealthRing(0);
    return;
  }
  const pick = candidates.reduce((lowest, current) => (
    Number(current.window.remainingPercent) < Number(lowest.window.remainingPercent) ? current : lowest
  ));
  const remaining = Math.max(0, Math.min(100, Number(pick.window.remainingPercent) || 0));
  const stateName = remaining < 20 ? 'critical' : remaining < 50 ? 'warning' : 'healthy';
  const providerId = pick.provider?.label || pick.provider?.provider || 'Quota';
  const providerName = String(providerId).replace(/^./, (letter) => letter.toUpperCase());
  const reset = formatReset(pick.window.resetsAt);
  els.quotaHealthSummary.dataset.state = stateName;
  els.quotaHealthPercent.textContent = `${Math.round(remaining)}%`;
  els.quotaHealthTitle.textContent = `${providerName} 可用`;
  els.quotaHealthReset.textContent = reset || '暂无重置时间';
  els.quotaHealthBadge.textContent = stateName === 'critical' ? '偏低' : stateName === 'warning' ? '关注' : '充足';
  drawQuotaHealthRing(remaining, stateName);
}
...
function renderSettingsAppUpdateRow() {
  if (!els.appUpdateInstalled) return;
  const s = state.appUpdate;
  if (!s) {
...
async function refreshTokscaleStatus() {
  if (!window.quotaPulse.getTokscaleStatus) return;
  try {
    state.tokscaleStatus = await window.quotaPulse.getTokscaleStatus();
...
async function checkTokscaleNpm() {
  ...
  renderTokscaleStatus();
  try {
    const result = await window.quotaPulse.checkTokscaleNpm();
...
async function downloadTokscaleFromNpm() {
  ...
  renderTokscaleStatus();
  try {
    const result = await window.quotaPulse.downloadTokscaleFromNpm();
    if (result?.error) throw new Error(result.error);
    mergeTokscalePayload(result);
...
async function resetTokscaleToBundled() {
  ...
  renderTokscaleStatus();
  try {
    state.tokscaleStatus = await window.quotaPulse.resetTokscaleToBundled();
...
function availableBreakdownIds() {
  const available = VIEW_DISPLAY_OPTIONS.map(({ id }) => id);
  return limitViewAvailable() ? available : available.filter((id) => id !== 'limits');
}
...
function limitStatusLabel(status) {
  ...
  if (status === 'notConfigured') return '未登录';
  if (status === 'noSyncedData') return '暂无同步数据';
  if (status === 'unauthorized') return '请重新登录';
  if (status === 'rateLimited') return '请求受限';
  if (status === 'sourceRateLimited') return '用量接口受限';
...
function limitProviderMeta(provider, provenance = null) {
  ...
  if (provider.stale) {
    const parts = ['数据已过期', formatUpdatedAge(provider.updatedAt)];
...
function formatLimitWindowValue(window, fillPercent, hasPercent, showUsed) {
  ...
  if (Number.isFinite(remaining)) {
    return window?.showMeter === false ? formatLimitAmount(remaining) : `${formatLimitAmount(remaining)} 剩余`;
  }
  const limit = Number(window?.limit);
  if (Number.isFinite(limit)) return `${formatLimitAmount(limit)} 上限`;
  return '';
}
...
function formatHomeLimitWindowValue(window, showUsed) {
  ...
  if (window?.kind === 'balance') {
    return `${formatMoney(window.amount, window.currency)} 剩余`;
  }
...
function localizedLimitWindowLabel(value) {
...
function limitWindowNode(label, window, color, tone = 1, valueOverride = null, detailText = '') {
  ...
  const name = document.createElement('span');
  name.textContent = localizedLimitWindowLabel(window?.label || label);
  const value = document.createElement('span');
...
function renderLimitProviderHead(id, label, provider, color, options = {}) {
  ...
  if (switchAccount && window.quotaPulse?.codex?.switchSystemAccount) {
    ...
    switchButton.addEventListener('click', async (event) => {
      ...
      renderLimits();
      try {
        const result = await window.quotaPulse.codex.switchSystemAccount(switchAccount.id);
        if (!result?.ok) {
          ...
          renderLimits();
          window.quotaPulse.codex.refreshAccountLimits(switchAccount.id).then((refreshResult) => {
...
function renderLimits() {
  ...
  const rows = limitProviderOrderApi
    .orderedLimitProviders(VISIBLE_LIMIT_PROVIDERS, state.settings?.limitProviderOrder)
...
function renderServiceStatus() {
  ...
  const rows = serviceStatusRows().map((provider) => {
    ...
    row.addEventListener('click', () => window.quotaPulse.openExternal?.(provider.pageUrl));
    const head = document.createElement('div');
...
async function refreshServiceStatus(options = {}) {
  if (!window.quotaPulse.getServiceStatus || state.serviceStatusBusy) return;
  state.serviceStatusBusy = true;
  renderServiceStatus();
  try {
    state.serviceStatus = await window.quotaPulse.getServiceStatus({ force: options.force === true, providerIds: visibleServiceProviderIds() });
  } catch (error) {
...
async function openSessionDetail({ client, sessionId, sessionCost, title }) {
  ...
  try {
    const detail = await window.quotaPulse.getSessionDetail({ client, sessionId, period: state.period, sessionCost });
...
function renderTrends() {
  const charts = window.QuotaPulseUsageCharts;
...
function syncSettingsButtonState(settingsOpen) {
  if (!els.settingsButton) return;
  const label = settingsOpen ? t('views.backHome') : t('trayMenu.settings');
  els.settingsButton.title = label;
  els.settingsButton.setAttribute('aria-label', label);
}
...
function openSettingsPanel() {
  ...
  els.settingsPanel.classList.remove('hidden');
  els.shell.classList.add('settings-open');
  if (!SETTINGS_SECTION_IDS.some((section) => state.settingsSections[section])) {
...
async function loadHomeHistory() {
  if (state.homeHistoryBusy || !window.quotaPulse.getDashboardHistory) return;
  ...
  try {
    ...
    // a newer one.
    fetchedHistory = await window.quotaPulse.getDashboardHistory();
    resolved = true;
  } catch (error) {
...
function viewSwitcherHint(id) {
  const chinese = currentLocale().startsWith('zh');
  const hints = chinese
    ? {
        home: '用量、额度与趋势概览',
        limits: '查看账号额度与重置时间',
    ...
    : {
        home: 'Usage, quota, and trend overview',
        limits: 'Account quotas and reset times',
        session: 'Conversation-by-conversation usage',
        tool: 'Usage grouped by AI tool',
        model: 'Usage grouped by model'
      };
  return hints[id] || '';
}
...
function updateViewSwitcherOpenState({ focusMenu = false, focusDisclosure = false } = {}) {
  ...
  const current = els.viewSwitcher.querySelector('.view-switcher-current');
  ...
  current?.setAttribute('aria-expanded', String(state.viewSwitcherOpen));
...
function renderViewSwitcher({ focusMenu = false, focusDisclosure = false } = {}) {
  ...
  menuHead.className = 'view-switcher-menu-head';
  const menuTitle = document.createElement('strong');
  menuTitle.textContent = currentLocale().startsWith('zh') ? '切换工作区' : 'Switch workspace';
  menuHead.append(menuTitle);
  menu.append(menuHead);
  const menuGrid = document.createElement('div');
  menuGrid.className = 'view-switcher-menu-grid';
  for (const id of order) {
    ...
    item.title = viewSwitcherHint(id);
    ...
    itemCopy.className = 'view-switcher-menu-copy';
    itemCopy.append(itemLabel);
...
function homeLimitWindowLabel(window, providerId = '', visibleWindows = []) {
  ...
  if (compactLabel) return localizedLimitWindowLabel(compactLabel);
  if (window?.kind === 'billing') {
    const label = String(window?.label || '').trim();
    if (label) return localizedLimitWindowLabel(label);
  }
  const key = {
  ...
  if (key) return localizedLimitWindowLabel(t(key));
  if (window?.kind === 'balance') return '余额';
  return localizedLimitWindowLabel(window.label);
}
...
function dailyWithHeatIntensity(daily) {
  return window.QuotaPulseUsageCharts.computeHeatmapIntensities(daily);
}
...
function render() {
  ...
  if (!contentReadySignaled) {
    ...
    window.quotaPulse.signalContentReady?.();
  }
}
...
async function refreshStats(options = {}) {
  ...
  try {
    state.stats = overlayAllTimeSessions(await window.quotaPulse.getStats(options));
    ...
    setStatus(statusTextFor(state.mode, state.streamConnected));
    if (!state.stats && els.breakdown) {
      const message = document.createElement('div');
      message.className = 'startup-error';
      const title = document.createElement('strong');
      title.textContent = '额度数据暂时无法加载';
...
function publishViewState() {
  window.quotaPulse.setViewState?.({ period: state.period, breakdown: state.breakdown });
}
...
const themePresetsApi = window.QuotaPulseThemePresets;
...
function renderFloatingBubbleContent() {
  ...
  if (window.QuotaPulseTrayText.isGeneratedTrayIconMode(mode)) {
    ...
    const fallback = state.stats && window.QuotaPulseTrayText.formatTrayText(state.stats, mode, currentCurrency(), state.settings);
    if (fallback) el.textContent = fallback;
    else renderFloatingBrandIcon(el);
  } else if (mode === 'icon') {
    el.classList.remove('bars');
    renderFloatingBrandIcon(el);
  } else {
    el.classList.remove('bars');
    el.textContent = state.stats ? (window.QuotaPulseTrayText.formatTrayText(state.stats, mode, currentCurrency(), state.settings) || '0') : '0';
  }
  reportFloatingBubbleSize();
}

function renderFloatingBrandIcon(el) {
  const img = new Image();
  img.className = 'floating-brand-icon';
  img.alt = '';
...
function reportFloatingBubbleSize() {
  ...
  if (mode !== 'icon' && el) {
    const pad = window.QuotaPulseTrayText.isGeneratedTrayIconMode(mode) ? 8 : BUBBLE_CONTENT_PAD_X * 2;
    width = Math.max(BUBBLE_CONTENT_MIN_W, Math.ceil(el.scrollWidth) + pad);
  }
  window.quotaPulse.setFloatingBubbleCollapsedSize?.({ width, height: BUBBLE_CONTENT_HEIGHT });
}
...
function handleFloatingBubbleHoverEnter() {
  ...
  floatingBubbleHoverRevealTimer = setTimeout(() => {
    ...
    window.quotaPulse.peekFloatingBubble?.();
...
function handleDocumentHoverLeave() {
  ...
  floatingBubbleHoverCollapseTimer = setTimeout(() => {
    ...
    if (!floatingBubbleHoverMode() || state.floatingBubble.collapsed) return;
    window.quotaPulse.collapseFloatingBubbleIfIdle?.();
...
function handleFloatingBubblePointerMove(event) {
  ...
  const move = window.quotaPulse.moveFloatingBubble?.({
...
function handleFloatingBubblePointerUp(event) {
  ...
  if (!drag) return;
  if (!drag.moved) window.quotaPulse.expandFloatingBubble?.();
  else {
...
function applyAppearanceFromControls() {
  ...
  syncSliderRows();
  window.quotaPulse.previewAppearance?.(patch).catch(() => {});
}
...
async function copyToClipboard(text, button) {
  try {
    if (window.quotaPulse.copyText) await window.quotaPulse.copyText(text);
...
async function refreshHubInfo() {
  if (!window.quotaPulse.getHubInfo) return;
  try {
    state.hubInfo = await window.quotaPulse.getHubInfo();
    renderHubStatus();
  } catch (_) { /* ignore */ }
}
...
function syncPushForm() {
  if (!state.settings) return;
  if (els.pushFeishuEnabled) els.pushFeishuEnabled.checked = Boolean(state.settings.pushFeishuEnabled);
  if (els.pushFeishuWebhook) els.pushFeishuWebhook.value = state.settings.pushFeishuWebhook || '';
  if (els.pushFeishuSignKey) els.pushFeishuSignKey.value = state.settings.pushFeishuSignKey || '';
  if (els.pushEmailEnabled) els.pushEmailEnabled.checked = Boolean(state.settings.pushEmailEnabled);
  if (els.pushEmailSmtpServer) els.pushEmailSmtpServer.value = state.settings.pushEmailSmtpServer || 'smtp.qq.com';
  if (els.pushEmailSmtpPort) els.pushEmailSmtpPort.value = String(state.settings.pushEmailSmtpPort || 465);
  if (els.pushEmailSender) els.pushEmailSender.value = state.settings.pushEmailSender || '';
  if (els.pushEmailAuthCode) els.pushEmailAuthCode.value = state.settings.pushEmailAuthCode || '';
  if (els.pushEmailRecipients) els.pushEmailRecipients.value = state.settings.pushEmailRecipients || '';
  if (els.pushAlertLowEnabled) els.pushAlertLowEnabled.checked = state.settings.pushAlertLowEnabled !== false;
  if (els.pushAlertLowThreshold) els.pushAlertLowThreshold.value = String(state.settings.pushAlertLowThreshold || 10);
  if (els.pushAlertWarningEnabled) els.pushAlertWarningEnabled.checked = state.settings.pushAlertWarningEnabled !== false;
  if (els.pushAlertWarningThreshold) els.pushAlertWarningThreshold.value = String(state.settings.pushAlertWarningThreshold || 30);
  if (els.pushAlertRestoredEnabled) els.pushAlertRestoredEnabled.checked = state.settings.pushAlertRestoredEnabled !== false;
  if (els.pushAlertRestoredThreshold) els.pushAlertRestoredThreshold.value = String(state.settings.pushAlertRestoredThreshold || 100);
  if (els.pushReportIntervalEnabled) els.pushReportIntervalEnabled.checked = state.settings.pushReportIntervalEnabled !== false;
  if (els.pushReportIntervalHours) els.pushReportIntervalHours.value = String(state.settings.pushReportIntervalHours || 5);
  if (els.pushReportDailyEnabled) els.pushReportDailyEnabled.checked = Boolean(state.settings.pushReportDailyEnabled);
  if (els.pushReportDailyTime) els.pushReportDailyTime.value = state.settings.pushReportDailyTime || '22:00';
}
...
function syncSettingsForm() {
  ...
  if (els.hubUrlInput) els.hubUrlInput.value = state.settings.hubUrl || '';
  if (els.secretInput) els.secretInput.value = state.settings.secret || '';
...
function renderWslPanel() {
  ...
  if ((status.detected || []).length > 0) {
    ...
    if (wslStatusPresentationApi.sqliteHelpClients(status).length > 0) {
      ...
      guide.addEventListener('click', () => window.quotaPulse.openExternal?.(QUOTA_PULSE_WSL_SQLITE_GUIDE_URL));
...
function renderToolPreferences() {
  ...
  const clients = clientDisplayPreferencesApi.orderedClients(VISIBLE_CLIENTS, state.settings?.clientDisplayOrder, state.settings?.pinnedClients);
...
function renderLimitProviderCheckboxes() {
  ...
  const providers = limitProviderOrderApi.orderedLimitProviders(VISIBLE_LIMIT_PROVIDERS, state.settings?.limitProviderOrder);
...
async function saveSettings(patch) {
  try {
    state.settings = await window.quotaPulse.updateSettings(patch);
  } catch (error) {
    console.error('Could not persist settings:', error);
    try { state.settings = await window.quotaPulse.getSettings(); } catch (_) {}
...
async function refreshInitialStreamStatus() {
  try {
    const status = await window.quotaPulse.getStreamStatus?.();
    if (!status) return;
...
async function finishStartupInBackground() {
  const appUpdateTask = Promise.resolve(window.quotaPulse.getAppUpdateState?.())
    .then((payload) => {
      if (!payload) return;
  ...
  const opencodeTask = Promise.resolve(window.quotaPulse.opencode.refresh?.())
    .then((result) => {
      if (result?.ok) return refreshStats();
      return null;
    });
  ...
  await Promise.allSettled([
    ...
    opencodeTask,
    refreshHubInfo(),
    refreshTokscaleStatus()
  ]);
  await deliverTrayProviderIcons();
}

async function init() {
  const [appInfoResult, settingsResult] = await Promise.allSettled([
    window.quotaPulse.getAppInfo?.(),
    window.quotaPulse.getSettings()
  ]);
  if (appInfoResult.status === 'fulfilled') state.appInfo = appInfoResult.value;
  ...
  window.quotaPulse.onAppUpdatePush?.((payload) => {
    state.appUpdate = payload;
    renderAppUpdatePill();
    renderSettingsAppUpdateRow();
    renderAutomaticAppUpdateControl();
  ...
  restartTimer();
  await Promise.allSettled([
    refreshInitialStreamStatus(),
    refreshStats()
  ]);
  restartTimer();
  updateTitleFit();
  runAfterFirstPaint(finishStartupInBackground);
}
...
els.settingsButton.addEventListener('click', (event) => {
  ...
  if (settingsOpen && !SETTINGS_SECTION_IDS.some((section) => state.settingsSections[section])) {
    setSettingsSectionExpanded('general', true);
  }
  syncSettingsButtonState(settingsOpen);
...
els.hubSecretRegenButton?.addEventListener('click', async () => {
  if (!window.quotaPulse.regenerateHubSecret) return;
  const info = await window.quotaPulse.regenerateHubSecret();
  state.hubInfo = info;
...
function setPushStatus(text) {
  if (els.pushStatusText) els.pushStatusText.textContent = text || '';
}

els.pushFeishuEnabled?.addEventListener('change', async () => { await saveSettings({ pushFeishuEnabled: els.pushFeishuEnabled.checked }); });
els.pushFeishuWebhook?.addEventListener('change', async () => { await saveSettings({ pushFeishuWebhook: els.pushFeishuWebhook.value.trim() }); });
els.pushFeishuSignKey?.addEventListener('change', async () => { await saveSettings({ pushFeishuSignKey: els.pushFeishuSignKey.value.trim() }); });
els.pushEmailEnabled?.addEventListener('change', async () => { await saveSettings({ pushEmailEnabled: els.pushEmailEnabled.checked }); });
els.pushEmailSmtpServer?.addEventListener('change', async () => { await saveSettings({ pushEmailSmtpServer: els.pushEmailSmtpServer.value.trim() || 'smtp.qq.com' }); });
els.pushEmailSmtpPort?.addEventListener('change', async () => { await saveSettings({ pushEmailSmtpPort: Number(els.pushEmailSmtpPort.value) || 465 }); });
els.pushEmailSender?.addEventListener('change', async () => { await saveSettings({ pushEmailSender: els.pushEmailSender.value.trim() }); });
els.pushEmailAuthCode?.addEventListener('change', async () => { await saveSettings({ pushEmailAuthCode: els.pushEmailAuthCode.value.trim() }); });
els.pushEmailRecipients?.addEventListener('change', async () => { await saveSettings({ pushEmailRecipients: els.pushEmailRecipients.value.trim() }); });
els.pushAlertLowEnabled?.addEventListener('change', async () => { await saveSettings({ pushAlertLowEnabled: els.pushAlertLowEnabled.checked }); });
els.pushAlertLowThreshold?.addEventListener('change', async () => { await saveSettings({ pushAlertLowThreshold: Math.max(1, Math.min(99, Number(els.pushAlertLowThreshold.value) || 10)) }); });
els.pushAlertWarningEnabled?.addEventListener('change', async () => { await saveSettings({ pushAlertWarningEnabled: els.pushAlertWarningEnabled.checked }); });
els.pushAlertWarningThreshold?.addEventListener('change', async () => { await saveSettings({ pushAlertWarningThreshold: Math.max(1, Math.min(99, Number(els.pushAlertWarningThreshold.value) || 30)) }); });
els.pushAlertRestoredEnabled?.addEventListener('change', async () => { await saveSettings({ pushAlertRestoredEnabled: els.pushAlertRestoredEnabled.checked }); });
els.pushAlertRestoredThreshold?.addEventListener('change', async () => { await saveSettings({ pushAlertRestoredThreshold: Math.max(1, Math.min(100, Number(els.pushAlertRestoredThreshold.value) || 100)) }); });
els.pushReportIntervalEnabled?.addEventListener('change', async () => { await saveSettings({ pushReportIntervalEnabled: els.pushReportIntervalEnabled.checked }); });
els.pushReportIntervalHours?.addEventListener('change', async () => { await saveSettings({ pushReportIntervalHours: Math.max(1, Math.min(168, Number(els.pushReportIntervalHours.value) || 5)) }); });
els.pushReportDailyEnabled?.addEventListener('change', async () => { await saveSettings({ pushReportDailyEnabled: els.pushReportDailyEnabled.checked }); });
els.pushReportDailyTime?.addEventListener('change', async () => { await saveSettings({ pushReportDailyTime: /^\d{1,2}:\d{2}$/.test(els.pushReportDailyTime.value.trim()) ? els.pushReportDailyTime.value.trim() : '22:00' }); });

els.pushFeishuTestButton?.addEventListener('click', async () => {
  setPushStatus(t('settings.push.testing'));
  try {
    const result = await window.quotaPulse.push.test('feishu');
    setPushStatus(result.ok ? t('settings.push.testOk') : t('settings.push.testFail', { error: result.error || '' }));
  } catch (error) {
    setPushStatus(t('settings.push.testFail', { error: error.message || String(error) }));
  }
});
els.pushEmailTestButton?.addEventListener('click', async () => {
  setPushStatus(t('settings.push.testing'));
  try {
    const result = await window.quotaPulse.push.test('email');
    setPushStatus(result.ok ? t('settings.push.testOk') : t('settings.push.testFail', { error: result.error || '' }));
  } catch (error) {
    setPushStatus(t('settings.push.testFail', { error: error.message || String(error) }));
  }
});
els.pushSendNowButton?.addEventListener('click', async () => {
  setPushStatus(t('settings.push.sending'));
  try {
    const result = await window.quotaPulse.push.sendNow();
    const detail = (result.channels && result.channels.length) ? result.channels.join(', ') : (result.errors && result.errors.join('；')) || '';
    setPushStatus(result.ok ? t('settings.push.sendOk', { channels: detail }) : t('settings.push.sendFail', { error: detail }));
  } catch (error) {
    setPushStatus(t('settings.push.sendFail', { error: error.message || String(error) }));
  }
});
window.quotaPulse.push.onResult((result) => {
  const detail = (result.channels && result.channels.length) ? result.channels.join(', ') : (result.errors && result.errors.join('；')) || '';
  setPushStatus(result.ok ? t('settings.push.sendOk', { channels: detail }) : t('settings.push.sendFail', { error: detail }));
});
...
els.clearSessionUsageArchiveButton?.addEventListener('click', async () => {
  ...
  try {
    const result = await window.quotaPulse.clearSessionUsageArchive();
    if (!result?.ok) {
...
els.exportPickDirButton?.addEventListener('click', async () => {
  const result = await window.quotaPulse.pickExportDir();
...
els.exportNowButton?.addEventListener('click', async () => {
  ...
  try {
    const result = await window.quotaPulse.exportNow();
    if (result?.ok) {
...
els.openConfigButton.addEventListener('click', () => window.quotaPulse.openUserData());
...
els.openTokscaleLinkButton?.addEventListener('click', () => window.quotaPulse.openExternal?.('https://github.com/junhoyeo/tokscale'));
els.openRepositoryButton?.addEventListener('click', () => window.quotaPulse.openExternal?.(QUOTA_PULSE_REPOSITORY_URL));
els.reportIssueButton?.addEventListener('click', () => window.quotaPulse.openExternal?.(QUOTA_PULSE_ISSUES_URL));
...
els.minButton.addEventListener('click', () => window.quotaPulse.minimize());
els.closeButton.addEventListener('click', () => window.quotaPulse.close());
els.trendsPanel.addEventListener('click', (event) => {
  if (event.target.closest('.trends-spark, .trends-open-hint')) window.quotaPulse.openDashboard();
});
els.trendsPanel.addEventListener('keydown', (event) => {
  if ((event.key === 'Enter' || event.key === ' ') && event.target.closest('.trends-spark')) {
    event.preventDefault();
    window.quotaPulse.openDashboard();
  }
});
...
els.floatingBubbleTab.addEventListener('keydown', (event) => {
  if (event.key !== 'Enter' && event.key !== ' ') return;
  event.preventDefault();
  window.quotaPulse.expandFloatingBubble?.();
});

async function runAppUpdateAction() {
  const mode = appUpdateActionMode(state.appUpdate);
  if (mode === 'install') {
    state.appUpdate = await window.quotaPulse.installAppUpdate();
  } else if (mode === 'download') {
    state.appUpdate = await window.quotaPulse.downloadAppUpdate();
  } else if (mode === 'release') {
    const latest = state.appUpdate?.latest;
    if (!latest?.htmlUrl) return;
    await window.quotaPulse.openExternal(latest.htmlUrl);
  } else {
    return;
  }
  renderAppUpdatePill();
...
els.appUpdatePillAction.addEventListener('click', async () => {
  if (!renderAppUpdatePopover(state.appUpdate) || typeof els.appUpdatePopover.showPopover !== 'function') {
    if (appUpdateActionMode(state.appUpdate) === 'install') {
      ...
      if (url) await window.quotaPulse.openExternal(url);
      return;
    }
...
els.appUpdatePillDismiss.addEventListener('click', async () => {
  ...
  if (!version) return;
  state.appUpdate = await window.quotaPulse.dismissAppUpdate(version);
  if (els.appUpdatePopover.matches(':popover-open')) els.appUpdatePopover.hidePopover();
  renderAppUpdatePill();
});
...
els.appUpdateCheckButton?.addEventListener('click', async () => {
  state.appUpdate = await window.quotaPulse.checkAppUpdateNow();
  renderAppUpdatePill();
  renderSettingsAppUpdateRow();
});
...
window.quotaPulse.onSettingsPush?.((next) => {
  if (!next) return;
...
window.quotaPulse.onOpenSettings?.(openSettingsPanel);
window.quotaPulse.onOpenView?.(openViewFromTray);

window.quotaPulse.onFloatingBubbleState?.((payload) => {
  applyFloatingBubbleState(payload);
});

window.quotaPulse.onHubPush?.((payload) => {
...
window.quotaPulse.onTokscalePush?.((payload) => {
  mergeTokscalePayload(payload);
  renderTokscaleStatus();
});

window.quotaPulse.onStatsPush?.((payload) => {
  if (!payload) return;
...
function pickWorstProvider(stats) {
  return window.QuotaPulseTrayText.pickWorstLimitProvider(stats);
}

function pickWorstSessionProvider(stats) {
  return window.QuotaPulseTrayText.pickLimitProviderByKindPriority(stats, ['session', 'weekly']);
}

function pickWorstWeeklyProvider(stats) {
  return window.QuotaPulseTrayText.pickWorstLimitProvider(stats, { kind: 'weekly' });
}
...
const trayProviderIconDeliveryGuard = window.QuotaPulseTrayProviderIcons.createTrayProviderIconDeliveryGuard();
const trayComposers = {};
...
function paintProviderImage(ctx, image, x, y, size, templateColor = '') {
  ...
  } = window.QuotaPulseTrayProviderIcons;
...
function renderBarsIcon(stats, height = 44, picker = pickWorstProvider, colors = {}, options = {}) {
  ...
  const { trayBarFillWidth, trayBarsLayout } = window.QuotaPulseTrayBars;
...
function pickConfiguredSessionProviders(stats, configOrder) {
  return window.QuotaPulseTrayText.pickConfiguredLimitProviders(stats, {
...
function renderLimitSessionsIcon(stats, height = 44, configOrder, colors = {}, options = {}) {
  ...
  const { trayBarsLayout } = window.QuotaPulseTrayBars;
  const layout = trayBarsLayout(height);
...
function drawTrayFallbackMark(ctx, value, x, y, size, color) {
  ...
  ctx.textBaseline = 'middle';
  ctx.fillText(value === 'app' ? 'QP' : String(value || '?').slice(0, 1).toUpperCase(), x + size / 2, y + size / 2 + 1);
  ctx.restore();
}
...
function drawCustomTrayProviderBadge(ctx, x, y, size, color) {
  const { trayProviderBadgeLayout } = window.QuotaPulseTrayProviderIcons;
...
async function maybeUpdateBarsIcon(options = {}) {
  ...
  if (!window.QuotaPulseTrayText.isGeneratedTrayIconMode(mode)) return;
  if (!window.quotaPulse.setTrayIcons) return;
  const dataUrl = trayDataUrlForMode(mode, 44);
  try { await window.quotaPulse.setTrayIcons({ [mode]: dataUrl || null }); } catch (_) {}
}
...
function trayComposerProviderIcon(provider) {
  ...
  if (id === 'app') return state.appInfo?.appIconDataUrl || '';
  return window.QuotaPulseTrayProviderIcons.trayProviderIconSources([id])[id] || '';
}
...
function createTrayComposer(surface) {
  ...
  return window.QuotaPulseTrayComposer.createTrayComposer({
    root,
    surface,
...
function syncTrayComposerVisibility() {
  ...
  window.QuotaPulseTrayComposer.syncTrayComposerSurfaces(
    surfaces,
    trayComposers,
    createTrayComposer
  );
...
async function deliverTrayProviderIcons(showBadge = state.settings?.showTrayProviderBadge === true) {
  ...
  const sources = window.QuotaPulseTrayProviderIcons.trayProviderIconSources(trayIconProviderIds);
  if (state.appInfo?.appIconDataUrl) sources.app = state.appInfo.appIconDataUrl;
  const icons = {};
  ...
  if (Object.keys(icons).length) await window.quotaPulse.setTrayIcons(icons);
  if (!trayProviderIconDeliveryGuard.isCurrent(deliveryId)) return;
...
function renderCodexAccounts() {
  ...
  if (accounts.length === 0) {
    ...
    for (const account of accounts) {
      ...
      input.addEventListener('change', async () => {
        input.disabled = true;
        const result = await window.quotaPulse.codex.setAccountEnabled(account.id, input.checked);
        if (!result?.ok) {
      ...
      remove.addEventListener('click', async () => {
        ...
        const result = await window.quotaPulse.codex.removeAccount(account.id);
        if (!result?.ok) {
...
async function refreshCodexAccounts() {
  try {
    state.settings.codexManagedAccounts = await window.quotaPulse.codex.accounts();
    state.codexAccountError = '';
  } catch (err) {
...
function renderMimoStatus() {
  ...
  if (accounts.length > 0) {
    for (const [index, account] of accounts.entries()) {
      ...
      input.addEventListener('change', async () => {
        input.disabled = true;
        const result = await window.quotaPulse.mimo.setAccountEnabled(account.id, input.checked);
        if (!result?.ok) {
      ...
      remove.addEventListener('click', async () => {
        ...
        const result = await window.quotaPulse.mimo.removeAccount(account.id);
        if (result?.ok) {
          state.mimoAccountError = '';
          state.settings.mimoManagedAccounts = result.accounts || [];
          renderMimoStatus();
...
function renderOpenCodeProfiles() {
  ...
  const api = window.quotaPulse.opencode;
...
function renderOpenRouterProfiles() {
  renderNamedApiProfiles({
    ...
    profileCountStateKey: 'openrouterProfileCount',
    api: window.quotaPulse.openrouter,
...
function renderThirdPartyProfiles() {
  renderNamedApiProfiles({
    ...
    profileCountStateKey: 'thirdPartyProfileCount',
    api: window.quotaPulse.thirdparty,
...
async function refreshCursorStatus() {
  ...
  renderCursorStatus();
  try {
    const status = await window.quotaPulse.cursor.status();
...
function setupCustomPricingUI() {
  ...
  select.addEventListener('change', async () => {
    ...
    try {
      const res = await window.quotaPulse.lookupModelPricing(id);
...
function setupCursorAccountUI() {
  ...
  if (codexToggle) {
    ...
    window.quotaPulse.codex.onLoginStatus((status) => {
    ...
    codexAddButton.addEventListener('click', async () => {
      ...
      renderCodexAccounts();
      try {
        const result = await window.quotaPulse.codex.addAccount({ flowId });
    ...
    codexCancelButton.addEventListener('click', async () => {
      ...
      if (!isCurrentCodexSignInFlow(flowId)) return;
      const result = await window.quotaPulse.codex.cancelLogin({ flowId });
    ...
    codexConfirmWorkspaceButton.addEventListener('click', async () => {
      ...
      try {
        const result = await window.quotaPulse.codex.selectWorkspace({ flowId, workspaceId });
    ...
    codexOpenUrlButton.addEventListener('click', async () => {
      ...
      const result = await window.quotaPulse.openExternal(state.codexLoginUrl);
      if (!result?.ok) {
  ...
  document.getElementById('cursorLoginButton').addEventListener('click', () => {
    window.quotaPulse.openExternal('https://cursor.com/settings');
  });

  document.getElementById('cursorLogoutButton').addEventListener('click', async () => {
    await window.quotaPulse.cursor.logout();
  ...
  document.getElementById('cursorManualSubmit').addEventListener('click', async () => {
    ...
    const errorEl = document.getElementById('cursorErrorMessage');
    errorEl.classList.add('hidden');
    const result = await window.quotaPulse.cursor.loginManual(input.value);
    if (!result.ok) {
  ...
  if (opencodeToggle) {
    ...
    document.getElementById('opencodeOpenBrowser')?.addEventListener('click', () => {
      window.quotaPulse.openExternal('https://opencode.ai/auth');
    });
    ...
    document.getElementById('opencodeCookieSubmit').addEventListener('click', async () => {
      ...
      const result = await window.quotaPulse.opencode.saveProfile(name, cookie);
      if (result.ok) {
        input.value = '';
        nameInput.value = '';
        renderOpenCodeProfiles();
  ...
  if (openrouterToggle) {
    ...
    document.getElementById('openrouterOpenBrowser')?.addEventListener('click', () => {
      window.quotaPulse.openExternal('https://openrouter.ai/settings/keys');
    });
    document.getElementById('openrouterProfileSubmit')?.addEventListener('click', async () => {
      ...
      const result = await window.quotaPulse.openrouter.saveProfile(name, apiKey);
      if (result?.ok) {
        nameInput.value = '';
        keyInput.value = '';
        renderOpenRouterProfiles();
  ...
  if (thirdpartyToggle) {
    ...
    document.getElementById('thirdpartyProfileSubmit')?.addEventListener('click', async () => {
      ...
      const apiKey = String(keyInput?.value || '').trim();
      errorEl?.classList.add('hidden');
      const result = await window.quotaPulse.thirdparty.saveProfile({
        name,
        adapter,
        baseUrl,
        accessToken,
  ...
  if (deepseekToggle) {
    ...
    document.getElementById('deepseekOpenBrowser').addEventListener('click', () => {
      window.quotaPulse.openExternal('https://platform.deepseek.com/api_keys');
    });
  ...
  if (minimaxToggle) {
    ...
    document.getElementById('minimaxOpenBrowser').addEventListener('click', () => {
      window.quotaPulse.openExternal(minimaxPlatformUrl());
    });
  ...
  if (zaiToggle) {
    ...
    document.getElementById('zaiOpenBrowser').addEventListener('click', () => {
      window.quotaPulse.openExternal(zaiPlatformUrl());
    });
  ...
  if (zaiteamToggle) {
    ...
    document.getElementById('zaiteamOpenBrowser').addEventListener('click', () => {
      window.quotaPulse.openExternal(zaiteamPlatformUrl());
    });
  ...
  if (volcengineToggle) {
    ...
    document.getElementById('volcengineOpenBrowser').addEventListener('click', () => {
      window.quotaPulse.openExternal(volcenginePlatformUrl());
    });
  ...
  if (claudeToggle) {
    ...
    document.getElementById('claudeOpenBrowser').addEventListener('click', () => {
      window.quotaPulse.openExternal(claudePlatformUrl());
    });
    ...
    document.getElementById('claudeWebCookieSubmit').addEventListener('click', async () => {
      ...
      try {
        const result = await window.quotaPulse.claude.saveCookie(input.value);
  ...
  if (qoderToggle) {
    ...
    document.getElementById('qoderOpenBrowser').addEventListener('click', () => {
      window.quotaPulse.openExternal(qoderPlatformUrl());
    });
  ...
  if (ollamaToggle) {
    ...
    document.getElementById('ollamaOpenBrowser').addEventListener('click', () => {
      window.quotaPulse.openExternal(ollamaPlatformUrl());
    });
    ...
    document.getElementById('ollamaCookieSubmit').addEventListener('click', async () => {
      ...
      try {
        ...
        renderExternalProviderStatus('ollama');
        const validation = await window.quotaPulse.ollama.validateCookie(input.value);
        if (!validation?.ok) {
          clearExternalProviderCheckPending('ollama');
          renderExternalProviderStatus('ollama');
  ...
  if (kimiToggle) {
    ...
    document.getElementById('kimiOpenBrowser').addEventListener('click', () => {
      window.quotaPulse.openExternal(kimiPlatformUrl());
    });
  ...
  if (mimoToggle) {
    ...
    window.quotaPulse.mimo.onAccounts((accounts) => {
      state.settings.mimoManagedAccounts = accounts || [];
      renderMimoStatus();
    });

    window.quotaPulse.mimo.accounts().then((accounts) => {
      state.settings.mimoManagedAccounts = accounts || [];
      renderMimoStatus();
    }).catch(() => {});

    document.getElementById('mimoOpenConsoleButton').addEventListener('click', async () => {
      const result = await window.quotaPulse.mimo.openConsole();
      if (!result?.ok) {
    ...
    document.getElementById('mimoSaveAccountButton').addEventListener('click', async () => {
      ...
      let result;
      try {
        result = await window.quotaPulse.mimo.addAccount(input.value);
      } catch (_) {
      ...
      input.value = '';
      state.mimoAccountError = '';
      state.settings.mimoManagedAccounts = await window.quotaPulse.mimo.accounts();
      renderMimoStatus();
  ...
  if (copilotToggle) {
    ...
    window.quotaPulse.copilot?.onLoginStatus?.((status) => {
      if (!status) return;
    ...
    document.getElementById('copilotSignInButton').addEventListener('click', async () => {
      ...
      renderCopilotStatus();
      try {
        const result = await window.quotaPulse.copilot.signIn({ flowId });
    ...
    document.getElementById('copilotCancelSignInButton').addEventListener('click', async () => {
      ...
      await window.quotaPulse.copilot.cancelSignIn({ flowId });
      if (!isCurrentCopilotSignInFlow(flowId)) return;
      state.copilotSignInBusy = false;
      state.copilotSignInCancelable = false;
      state.copilotSignInFlowId = '';
...
function showStartupFailure(error) {
  ...
  const host = els.breakdown || document.querySelector('.shell') || document.body;
  const message = document.createElement('div');
  message.className = 'startup-error';
  const title = document.createElement('strong');
  title.textContent = '界面初始化失败';
  const detail = document.createElement('span');
  detail.textContent = error?.message || String(error);
  message.append(title, detail);
...
try {
  ...
  void init().catch(showStartupFailure);
} catch (error) {
