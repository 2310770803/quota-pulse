...
Quota Pulse is local-first. It processes AI-tool usage logs on the device and does not send analytics or telemetry to the project maintainer. The project does not operate a hosted data-collection service.

## Network features

Quota Pulse makes network requests only for documented or user-enabled features:
...
Multi-device sync is optional and has no Quota Pulse-operated default server. The operator chooses and controls the destination hub, whether it runs in the app, on a self-hosted Node server, or on Cloudflare Workers.
