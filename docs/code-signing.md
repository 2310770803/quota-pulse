...
The current release workflow Authenticode-signs Quota Pulse's Windows application executable, installer (`Quota-Pulse-Setup-*.exe`), and portable build (`Quota-Pulse-*.exe`). The expected publisher is **SignPath Foundation**.
...
```powershell
Get-AuthenticodeSignature ".\Quota-Pulse-Setup-<version>.exe", ".\Quota-Pulse-<version>.exe" | Format-List Path, Status, SignerCertificate, TimeStamperCertificate
```

A genuine Quota Pulse release should show `Valid` status, SignPath Foundation as the signer, and timestamp information.

After installation, the same check can be run against `Quota Pulse.exe` in the application's installation directory.
...
- Privacy policy: [Quota Pulse privacy policy](privacy.md)
