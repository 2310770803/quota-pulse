...
Quota Pulse has two configuration surfaces:
...
New configuration uses the `QUOTA_PULSE_*` prefix. Existing `TOKEN_MONITOR_*` variables remain accepted as compatibility aliases, while an explicitly set `QUOTA_PULSE_*` value always wins.

---

## Widget (GUI)

Click the `⚙` button in the top-right corner of the widget to open the two-pane settings center. Choose a destination in the left rail; the corresponding controls appear in the right workspace:
...
```env
QUOTA_PULSE_HUB_URL=               # required in sync mode — Worker URL or http://<lan-ip>:17321
QUOTA_PULSE_SECRET=                # shared secret; must match the hub
QUOTA_PULSE_DEVICE_ID=             # optional — defaults to the hostname
QUOTA_PULSE_SYNC_UPLOAD_INTERVAL_MS= # optional — 0/live, 600000/10min, 1200000/20min, 1800000/30min
QUOTA_PULSE_CLIENTS=               # optional — defaults to all supported tools; empty disables tracking
QUOTA_PULSE_PROJECTS_ENABLED=      # optional — defaults off; 1 collects project metadata
QUOTA_PULSE_HISTORY_ENABLED=       # optional — defaults on; 0 skips trend history
QUOTA_PULSE_SESSION_USAGE_ARCHIVE_ENABLED= # optional — defaults on; 0 stops archiving deleted-session usage
QUOTA_PULSE_LIMITS_ENABLED=        # optional — defaults on; 0 skips CLI probing
QUOTA_PULSE_LIMIT_PROVIDERS=       # optional — defaults to all supported providers
```
