...
Quota Pulse can export your usage data in **tool-agnostic** formats so you can
...
- **Export data…** — pick a folder; Quota Pulse writes the file set below into
  it once, right now.
...
| File | What it is |
|---|---|
| `quota-pulse-export.json` | Complete, lossless snapshot + history in one JSON object |
| `quota-pulse-snapshot.csv` | Current totals (today / month / all-time), one row per tool and per model |
| `quota-pulse-daily.csv` | Daily time-series **history**, one row per day × tool — spans your whole tracked history, not just today (only written when trend history has data) |
...
`quota-pulse-snapshot.csv` under `period,today`, not here.
...
### `quota-pulse-export.json`

```json
{
  "generatedAt": "2026-07-03T14:30:00.000Z",
  "app": { "name": "quota-pulse", "version": "0.19.0" },
...
Point auto-export at a folder inside your vault (e.g. `QuotaPulse/`), then in a
note:

````markdown
```dataviewjs
const raw = await app.vault.adapter.read("QuotaPulse/quota-pulse-export.json");
...
Prefer a code-free table? Import `quota-pulse-daily.csv` with a CSV/table
plugin instead.

### Excel / Google Sheets / Numbers

Open `quota-pulse-snapshot.csv` or `quota-pulse-daily.csv` directly. Both are
tidy (long) tables, so they pivot cleanly.

### Grafana / dashboards

Use `quota-pulse-export.json` (or the CSVs) as a JSON/CSV file data source. For
