...
<div align="center">
    <img src="assets/brand-icon.png" alt="Quota Pulse logo" width="120">
    <h1>Quota Pulse</h1>
</div>
...
<p align="center">
    <a href="https://github.com/2310770803/quota-pulse/releases"><img src="https://img.shields.io/github/v/release/2310770803/quota-pulse?include_prereleases&style=flat-square&label=release&color=22c55e" alt="Latest release" /></a>
    <a href="https://github.com/2310770803/quota-pulse/releases"><img src="https://img.shields.io/github/downloads/2310770803/quota-pulse/total?style=flat-square&color=22c55e" alt="Total downloads" /></a>
...
<table>
<tr>
<td width="435" align="center"><img src=".github/assets/quota-pulse-main.png" width="400" alt="Quota Pulse main view"><br><sub>Live token totals, cost, quota, and model breakdown</sub></td>
<td width="435" align="center"><img src=".github/assets/quota-pulse-settings.png" width="400" alt="Quota Pulse settings"><br><sub>Compact settings workspace with version 1.0.0</sub></td>
</tr>
</table>
...
Quota Pulse was informed by [Token Monitor](https://github.com/Javis603/token-monitor) during its early product and architecture exploration. It has since developed its own brand, settings architecture, interface, and workspace navigation.

## Supported Tools

Quota Pulse supports token usage, account-limit checks, and session details separately:
...
Quota Pulse is a compact desktop workspace for monitoring local AI coding usage, account limits, and session details.
...
- **Preserve deleted session usage** — many tools prune old sessions (Claude Code drops transcripts after 30 days by default), losing that history. When enabled, Quota Pulse archives observed daily tool/model usage locally so the heatmap and trends survive even after the source files are gone (see [Session data retention](#session-data-retention) below)
...
Download from [GitHub Releases](https://github.com/2310770803/quota-pulse/releases).
...
Background monitoring runs while Quota Pulse is running — quitting (not just closing the window) stops it.
...
cp .env.example .env
# set QUOTA_PULSE_SECRET to something private, then:
npm run hub
```

#### Option C — Cloudflare Worker hub (across networks, including iPhone)

[![Deploy to Cloudflare](https://deploy.workers.cloudflare.com/button)](https://deploy.workers.cloudflare.com/?url=https://github.com/2310770803/quota-pulse/tree/main/worker)

One-click deploy — Cloudflare will prompt for the `QUOTA_PULSE_SECRET` during setup. Or deploy manually:
...
With **Preserve deleted session usage** enabled (Settings → Collection), Quota Pulse archives observed daily tool/model usage locally with no time limit — so even after a source tool prunes its own sessions, the heatmap and trends are unaffected.
...
This archive only covers days Quota Pulse has already observed; data deleted before it started tracking cannot be recovered.

</details>

## Settings

There are two places to configure Quota Pulse; day-to-day use only needs the first:

- **Widget (GUI)** — click the `⚙` button in the top-right corner. Settings open as a two-pane control center: choose a destination in the left navigation, then work on one focused section on the right. The lower-left workspace launcher switches between Home, Limits, Sessions, Tools, and Models. The `⇧` button in the title bar cycles the window behavior.
...
- [Token Monitor](https://github.com/Javis603/token-monitor) was an early reference for product scope and parts of the initial architecture. Quota Pulse now carries its own brand, settings information architecture, main presentation, and workspace navigation.
