...
- **macOS Apple Silicon** — [Quota-Pulse-0.35.0-arm64.dmg](https://github.com/Javis603/quota-pulse/releases/download/v0.35.0/Quota-Pulse-0.35.0-arm64.dmg)
- **macOS Intel** — [Quota-Pulse-0.35.0-x64.dmg](https://github.com/Javis603/quota-pulse/releases/download/v0.35.0/Quota-Pulse-0.35.0-x64.dmg)
- **Windows Installer** — [Quota-Pulse-Setup-0.35.0.exe](https://github.com/Javis603/quota-pulse/releases/download/v0.35.0/Quota-Pulse-Setup-0.35.0.exe) (recommended)
- **Windows Portable** — [Quota-Pulse-0.35.0.exe](https://github.com/Javis603/quota-pulse/releases/download/v0.35.0/Quota-Pulse-0.35.0.exe) (no install required)
- **Linux x64** — [Quota-Pulse-0.35.0.AppImage](https://github.com/Javis603/quota-pulse/releases/download/v0.35.0/Quota-Pulse-0.35.0.AppImage)
...
**macOS:** the app is Developer ID-signed and notarized by Apple. Open the `.dmg`, then drag Quota Pulse to Applications.

**Windows:** both executables are signed ([how to verify](https://github.com/Javis603/quota-pulse/blob/main/docs/code-signing.md#verify-a-download)), but you may still see a brief SmartScreen prompt on the first few releases while the certificate builds reputation with Microsoft — More info → Run anyway.

**Linux:** mark the AppImage executable, then run it:

```bash
chmod +x "Quota Pulse"*.AppImage
./"Quota Pulse"*.AppImage
```

### Other notes

Other platforms are not pre-built — run from source per the [README](https://github.com/Javis603/quota-pulse#readme). The macOS `.zip` is the same app repackaged; ignore it unless you specifically need it.
...
- **macOS Apple Silicon** — [Quota-Pulse-0.35.0-arm64.dmg](https://github.com/Javis603/quota-pulse/releases/download/v0.35.0/Quota-Pulse-0.35.0-arm64.dmg)
- **macOS Intel** — [Quota-Pulse-0.35.0-x64.dmg](https://github.com/Javis603/quota-pulse/releases/download/v0.35.0/Quota-Pulse-0.35.0-x64.dmg)
- **Windows 安装版** — [Quota-Pulse-Setup-0.35.0.exe](https://github.com/Javis603/quota-pulse/releases/download/v0.35.0/Quota-Pulse-Setup-0.35.0.exe)（推荐）
- **Windows 便携版** — [Quota-Pulse-0.35.0.exe](https://github.com/Javis603/quota-pulse/releases/download/v0.35.0/Quota-Pulse-0.35.0.exe)（免安装）
- **Linux x64** — [Quota-Pulse-0.35.0.AppImage](https://github.com/Javis603/quota-pulse/releases/download/v0.35.0/Quota-Pulse-0.35.0.AppImage)
...
**macOS：** 应用已使用 Developer ID 签名并通过 Apple 公证。打开 `.dmg`，然后把 Quota Pulse 拖到 Applications。

**Windows：** 两个可执行文件均已签名（[查看验证方法](https://github.com/Javis603/quota-pulse/blob/main/docs/code-signing.md#verify-a-download)），但在证书刚建立信誉的最初几个版本，仍可能短暂出现 SmartScreen 提示 → 更多信息 → 仍要运行。
...
其他平台暂不提供预构建版本，请参考 [README](https://github.com/Javis603/quota-pulse#readme) 从源码运行。macOS 的 `.zip` 只是同一个 app 的重新打包版本，除非你明确需要，否则可以忽略。
