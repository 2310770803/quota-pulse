...
test('same-day anchor can paint a complete cached snapshot before tokscale finishes', () => {
  const updatedAt = '2026-07-28T02:03:04.000Z';
  const wslToday = emptyPeriod();
  wslToday.totalTokens = 7;
  wslToday.clients = { claude: 7 };
  const preview = cachedAnchorPreview({
    anchor: {
    ...
    wslStatus: { enabled: true, detected: ['Ubuntu'] },
    updatedAt,
    clients: 'claude',
  ...
  assert.equal(preview.updatedAt, updatedAt);
  assert.equal(preview.today.totalTokens, 57);
  assert.equal(preview.month.totalTokens, 50);
  assert.equal(preview.allTime.totalTokens, 50);
  assert.deepEqual(preview.trackedClients, ['claude']);
  assert.equal(preview.clientStatus.claude, 'active');
  assert.deepEqual(preview.wslStatus.detected, ['Ubuntu']);
});
...
test('restart reuse: anchor file on disk enables todayOnly on first interval tick', async () => {
  ...
  const originalSharedDir = process.env.QUOTA_PULSE_SHARED_DIR;
  process.env.QUOTA_PULSE_SHARED_DIR = tmpShared;
  let handle;
  try {
    ...
    if (originalSharedDir === undefined) delete process.env.QUOTA_PULSE_SHARED_DIR;
    else process.env.QUOTA_PULSE_SHARED_DIR = originalSharedDir;
