...
function apiFetch(keyBodies, creditsBodies = {}) {
  return async (url, init) => {
    assert.equal(init.headers['HTTP-Referer'], 'https://github.com/Javis603/quota-pulse');
    assert.equal(init.headers['X-OpenRouter-Title'], 'Quota Pulse');
...
test('openrouterToken prefers explicit, then Quota Pulse env, then standard env', () => {
  assert.equal(openrouterToken({ QUOTA_PULSE_OPENROUTER_API_KEY: 'tm', OPENROUTER_API_KEY: 'std' }, '"explicit"'), 'explicit');
  assert.equal(openrouterToken({ QUOTA_PULSE_OPENROUTER_API_KEY: 'tm', OPENROUTER_API_KEY: 'std' }), 'tm');
