...
test('New API helpers accept only the explicit Quota Pulse environment surface', () => {
  const env = {
    QUOTA_PULSE_NEWAPI_BASE_URL: '"https://example.com/v1/"',
    QUOTA_PULSE_NEWAPI_ACCESS_TOKEN: "'access-token'",
    QUOTA_PULSE_NEWAPI_USER_ID: '"42"',
    QUOTA_PULSE_NEWAPI_API_KEY: "'api-key'",
...
test('environment configuration prefers account quota and falls back to token quota', () => {
  assert.deepEqual(configuredAccounts({}, {
    env: {
      QUOTA_PULSE_NEWAPI_BASE_URL: 'https://compatible-env.example',
      QUOTA_PULSE_NEWAPI_ACCESS_TOKEN: 'access-only'
    }
  }), [{
  ...
  assert.deepEqual(configuredAccounts({}, {
    env: {
      QUOTA_PULSE_NEWAPI_BASE_URL: 'https://env.example/v1',
      QUOTA_PULSE_NEWAPI_ACCESS_TOKEN: 'access',
      QUOTA_PULSE_NEWAPI_USER_ID: '12',
      QUOTA_PULSE_NEWAPI_API_KEY: 'api-key'
    }
  }), [{
    name: THIRD_PARTY_ENV_ACCOUNT_NAME,
