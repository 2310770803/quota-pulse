...
test('appUpdateInstallSupport only enables packaged auto-updatable targets', () => {
  ...
  assert.deepEqual(appUpdateInstallSupport({
    isPackaged: true,
    platform: 'win32',
    env: { PORTABLE_EXECUTABLE_FILE: 'C:\\Downloads\\Quota-Pulse.exe' }
  ...
  assert.deepEqual(appUpdateInstallSupport({ isPackaged: true, platform: 'linux', env: { APPIMAGE: '/tmp/Quota Pulse.AppImage' } }), { supported: true, reason: '' });
});
...
test('parseLatestReleasePayload returns normalized object for valid payload', () => {
  const result = parseLatestReleasePayload({
    tag_name: 'v0.1.3',
    name: 'Quota Pulse 0.1.3',
    html_url: 'https://github.com/Javis603/quota-pulse/releases/tag/v0.1.3',
...
`
  ...
  assert.deepEqual(result, {
    ...
    tag: 'v0.1.3',
    name: 'Quota Pulse 0.1.3',
    htmlUrl: 'https://github.com/Javis603/quota-pulse/releases/tag/v0.1.3',
