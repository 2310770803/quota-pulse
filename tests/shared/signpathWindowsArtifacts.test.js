...
const APPLICATION = 'Quota Pulse.exe';
const INSTALLER = `Quota-Pulse-Setup-${VERSION}.exe`;
const PORTABLE = `Quota-Pulse-${VERSION}.exe`;
...
test('SignPath configurations restrict every signed PE to the release product metadata', () => {
  ...
  assert.deepEqual(openingTagAttributes(artifactXml, 'pe-file'), [
    {
      path: 'installer/Quota-Pulse-Setup-${version}.exe',
      'product-name': pkg.productName,
      'product-version': '${version}'
    },
    {
      path: 'portable/Quota-Pulse-${version}.exe',
      'product-name': pkg.productName,
      'product-version': '${version}'
    }
  ]);
...
function makeFixture(t) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'quota-pulse-signpath-'));
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      name: 'quota-pulse',
      version: VERSION,
      productName: 'Quota Pulse',
      build: {
        win: {
          verifyUpdateCodeSignature: true,
          signtoolOptions: { publisherName: 'SignPath Foundation' }
        },
        nsis: { artifactName: 'Quota-Pulse-Setup-${version}.${ext}' },
        portable: { artifactName: 'Quota-Pulse-${version}.${ext}' },
        publish: [{ provider: 'github', owner: 'Javis603', repo: 'quota-pulse' }]
      }
    })
  );
...
test('writes the updater config skipped by electron-builder prepackaged mode', (t) => {
  ...
  const expected = [
    ...
    'repo: "quota-pulse"',
    'provider: "github"',
    'updaterCacheDirName: "quota-pulse-updater"',
    'publisherName:',
    '  - "SignPath Foundation"',
    ''
  ].join('\n');
...
test('expectedWindowsArtifacts rejects unsafe output names and output-parameter versions', (t) => {
  ...
  pkg.version = VERSION;
  pkg.build.portable.artifactName = '..\\Quota-Pulse-${version}.${ext}';
  fs.writeFileSync(fixture.packageJsonPath, JSON.stringify(pkg));
