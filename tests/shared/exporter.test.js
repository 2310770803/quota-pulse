...
test('renderExportJson has the documented shape', () => {
  const json = JSON.parse(renderExportJson({ periods: PERIODS, history: HISTORY, meta: { generatedAt: 'X', app: { name: 'quota-pulse', version: '1.2.3' } } }));
  assert.equal(json.generatedAt, 'X');
  assert.deepEqual(json.app, { name: 'quota-pulse', version: '1.2.3' });
...
test('exportFileSet omits daily.csv when history has no daily rows', () => {
  ...
  assert.deepEqual(withDaily.map((f) => f.name), ['quota-pulse-export.json', 'quota-pulse-snapshot.csv', 'quota-pulse-daily.csv']);

  const noDaily = exportFileSet({ periods: PERIODS, history: { daily: [], monthly: [] } });
  assert.deepEqual(noDaily.map((f) => f.name), ['quota-pulse-export.json', 'quota-pulse-snapshot.csv']);
});
