...
function macUpdaterMetadata(version, arch) {
  return [
    `version: ${version}`,
    'files:',
    `  - url: Quota-Pulse-${version}-${arch}.zip`,
    `    sha512: ${arch}-zip-hash`,
    '    size: 100',
    `  - url: Quota-Pulse-${version}-${arch}.dmg`,
    `    sha512: ${arch}-dmg-hash`,
    '    size: 200',
    `path: Quota-Pulse-${version}-${arch}.zip`,
    `sha512: ${arch}-zip-hash`,
...
test('release artifact templates use GitHub-safe names', () => {
  ...
  assert.deepEqual(patterns, [
    'Quota-Pulse-${version}-${arch}.${ext}',
    'Quota-Pulse-${version}.${ext}',
    'Quota-Pulse-Setup-${version}.${ext}',
    'Quota-Pulse-${version}.${ext}'
  ]);
...
test('mac release scripts build native Apple Silicon and Intel artifacts', () => {
  ...
  assert.match(workflow, /artifacts\/quota-pulse-mac-arm64\/latest-mac\.yml \\\s+artifacts\/quota-pulse-mac-x64\/latest-mac\.yml/);
  ...
  const intelDmg = `Quota-Pulse-${rootPackage.version}-x64.dmg`;
...
test('extracts updater artifact names from url and path fields', () => {
  const names = referencedArtifactNames([
    'files:',
    '  - url: Quota-Pulse-0.25.0-arm64.zip',
    'path: "Quota-Pulse-0.25.0-arm64.zip"',
    "  - url: 'https://example.com/Quota-Pulse-0.25.0-arm64.dmg'"
  ].join('\n'));
  assert.deepEqual(names, [
    'Quota-Pulse-0.25.0-arm64.zip',
    'Quota-Pulse-0.25.0-arm64.dmg'
  ]);
});

test('fails when updater metadata references an asset that will not be uploaded', (t) => {
  const distDir = fs.mkdtempSync(path.join(os.tmpdir(), 'quota-pulse-release-'));
  ...
  assert.throws(
    ...
    /latest-mac\.yml -> Quota-Pulse-0\.25\.0-arm64\.zip/
  );

  fs.writeFileSync(path.join(distDir, 'Quota-Pulse-0.25.0-arm64.zip'), 'artifact');
...
test('merges arm64 and x64 mac updater files into one architecture-aware feed', (t) => {
  ...
  assert.deepEqual(referencedArtifactNames(merged), [
    `Quota-Pulse-${version}-arm64.zip`,
    `Quota-Pulse-${version}-arm64.dmg`,
    `Quota-Pulse-${version}-x64.zip`,
    `Quota-Pulse-${version}-x64.dmg`
  ]);
  assert.match(merged, new RegExp(`^path: Quota-Pulse-${version}-arm64\\.zip$`, 'm'));
  ...
  assert.deepEqual(
    ...
    [`Quota-Pulse-${version}-arm64.zip`, `Quota-Pulse-${version}-arm64.dmg`]
  );
  assert.deepEqual(
    MacUpdater.filterFilesForArch(files, false).map((file) => path.basename(file.url.pathname)),
    [`Quota-Pulse-${version}-x64.zip`, `Quota-Pulse-${version}-x64.dmg`]
  );

  const distDir = fs.mkdtempSync(path.join(os.tmpdir(), 'quota-pulse-mac-release-'));
  t.after(() => fs.rmSync(distDir, { recursive: true, force: true }));
...
test('rejects stale or missing top-level mac updater paths', () => {
  ...
  assert.throws(
    () => mergeMacUpdaterMetadata(
      arm64Metadata.replace(
        `path: Quota-Pulse-${version}-arm64.zip`,
        `path: Quota-Pulse-${version}-x64.zip`
      ),
      x64Metadata
    ),
    /arm64 metadata path Quota-Pulse-0\.33\.0-x64\.zip does not reference an arm64 artifact/
  );
  assert.throws(
    () => mergeMacUpdaterMetadata(
      arm64Metadata.replace(
  ...
  assert.throws(
    () => mergeMacUpdaterMetadata(
      arm64Metadata.replace(
        `path: Quota-Pulse-${version}-arm64.zip`,
        `path: Quota-Pulse-${version}-arm64.dmg`
      ),
      x64Metadata
    ),
    /arm64 metadata path Quota-Pulse-0\.33\.0-arm64\.dmg is not a zip artifact/
  );
  assert.throws(
    () => mergeMacUpdaterMetadata(
      arm64Metadata.replace(`path: Quota-Pulse-${version}-arm64.zip\n`, ''),
      x64Metadata
    ),
