...
test('Codex command candidates preserve an explicit command override on macOS', () => {
  assert.deepEqual(
    codexCommandCandidates({ QUOTA_PULSE_CODEX_COMMAND: '/custom/codex' }, 'darwin'),
    ['/custom/codex']
  );
});
...
test('fetchCodexLimits returns one provider per managed Codex account', async () => {
  ...
  const providers = await fetchCodexLimits({
    codexManagedAccounts: [
      { id: 'one', email: 'one@example.com', homePath: '/tmp/quota-pulse-codex/one' },
      { id: 'two', email: 'two@example.com', homePath: '/tmp/quota-pulse-codex/two' }
    ]
  }, {
  ...
  assert.deepEqual(seenHomes, ['/tmp/quota-pulse-codex/one', '/tmp/quota-pulse-codex/two']);
    ...
    codexManagedAccounts: [
      {
        ...
        homePath: '/tmp/quota-pulse-codex/personal'
      },
      {
        id: 'team',
        ...
        workspaceLabel: 'Team',
        homePath: '/tmp/quota-pulse-codex/team'
      }
    ]
  }, {
    now: () => Date.parse('2026-06-01T00:00:00Z'),
  ...
  const accounts = [
    {
      ...
      workspaceAccountId: 'workspace-team',
      homePath: '/tmp/quota-pulse-codex/one',
      authPath: '/tmp/quota-pulse-codex/one/auth.json'
    },
    {
      id: 'two',
      email: 'two@example.com',
      workspaceAccountId: 'workspace-team',
      homePath: '/tmp/quota-pulse-codex/two',
      authPath: '/tmp/quota-pulse-codex/two/auth.json'
    }
  ];
  const providers = await fetchCodexLimits({
    includeLiveCodexAccount: false,
...
test('fetchCodexLimits can refresh only the requested managed Codex account', async () => {
  ...
  const providers = await fetchCodexLimits({
    ...
    codexManagedAccounts: [
      { id: 'other', accountKey: 'sha256:other', email: 'other@example.com', homePath: '/tmp/quota-pulse-codex/other' },
      { id: 'target', accountKey: 'sha256:target', email: 'target@example.com', homePath: '/tmp/quota-pulse-codex/target' }
    ]
  }, {
    now: () => Date.parse('2026-06-01T00:00:00Z'),
    env: { PATH: '/usr/bin' },
  ...
  assert.deepEqual(seenHomes, ['/tmp/quota-pulse-codex/target']);
...
test('fetchCodexLimits skips disabled managed Codex accounts', async () => {
  ...
  const providers = await fetchCodexLimits({
    codexManagedAccounts: [
      { id: 'one', email: 'one@example.com', homePath: '/tmp/quota-pulse-codex/one', enabled: true },
      { id: 'two', email: 'two@example.com', homePath: '/tmp/quota-pulse-codex/two', enabled: false }
    ]
  }, {
    now: () => Date.parse('2026-06-01T00:00:00Z'),
    env: { PATH: '/usr/bin' },
  ...
  assert.deepEqual(seenHomes, ['/tmp/quota-pulse-codex/one']);
  assert.equal(providers.length, 1);
...
test('fetchCodexLimits keeps the live system account visible alongside managed accounts', async () => {
  ...
  assert.deepEqual(seenHomes, ['<live>', '/tmp/quota-pulse-codex/two']);
...
test('fetchCodexLimits does not show the live account twice when it is also managed', async () => {
  const providers = await fetchCodexLimits({
    codexManagedAccounts: [
      { id: 'a', email: 'a@example.com', homePath: '/tmp/quota-pulse-codex/a' }
    ]
  }, {
    now: () => Date.parse('2026-06-01T00:00:00Z'),
    env: { PATH: '/usr/bin' },
...
test('fetchCodexLimits dedups the live account against the same managed account by account id (no email needed)', async () => {
  ...
  const providers = await fetchCodexLimits({
    codexManagedAccounts: [
      { id: 'm', email: '', accountKey: sharedKey, homePath: '/tmp/quota-pulse-codex/m' }
    ]
  }, {
    now: () => Date.parse('2026-06-01T00:00:00Z'),
    env: { PATH: '/usr/bin' },
...
test('fetchCodexLimits fills the live account email from auth.json when the RPC omits it', async () => {
  ...
  const providers = await fetchCodexLimits({
    codexManagedAccounts: [
      { id: 'm', email: 'managed@example.com', accountKey: 'sha256:managed', homePath: '/tmp/quota-pulse-codex/m' }
    ]
  }, {
    now: () => Date.parse('2026-06-01T00:00:00Z'),
    env: { PATH: '/usr/bin' },
...
test('fetchCodexLimits augments reset credits expiry from the Codex OAuth endpoint', async () => {
  ...
  const providers = await fetchCodexLimits({}, {
    now: () => Date.parse('2026-06-30T00:00:00Z'),
    env: { PATH: '/usr/bin', CODEX_HOME: '/tmp/quota-pulse-codex/live' },
    codexAuthPath: '/tmp/quota-pulse-codex/live/auth.json',
    codexCommand: 'codex',
    readFileSync: (file) => {
