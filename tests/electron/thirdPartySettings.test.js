...
test('third-party fallback stays last after named providers across product surfaces', () => {
  ...
  const envProviderList = env.slice(
    ...
    env.indexOf('QUOTA_PULSE_LIMIT_PROVIDERS=')
  );
...
test('third-party adapters document New API compatibility, Custom, assets, and environment variables', () => {
  ...
  const env = read('.env.example');
  assert.match(env, /QUOTA_PULSE_NEWAPI_BASE_URL=/);
  assert.match(env, /QUOTA_PULSE_NEWAPI_ACCESS_TOKEN=/);
  assert.match(env, /QUOTA_PULSE_NEWAPI_USER_ID=/);
  assert.match(env, /QUOTA_PULSE_NEWAPI_API_KEY=/);
