...
test('renderer fetches service status only through preload IPC', () => {
  ...
  assert.match(refreshBody, /window\.quotaPulse\.getServiceStatus/);
...
test('workspace launcher opens a descriptive grid without cycle or hover-only behavior', () => {
  const html = readRendererFile('index.html');
  const app = readRendererFile('app.js');
  const css = readRendererFile('styles.css');
  ...
  assert.doesNotMatch(app, /function nextBreakdown/);
  assert.match(app, /function updateViewSwitcherOpenState/);
  assert.match(app, /function viewSwitcherHint/);
  ...
  assert.doesNotMatch(app, /view-switcher-kicker/);
  assert.match(app, /view-switcher-menu-item/);
  assert.match(app, /view-switcher-menu-head/);
  assert.match(app, /view-switcher-menu-grid/);
  assert.match(app, /view-switcher-menu-hint/);
  assert.match(app, /event\.key === 'Escape'/);
  assert.doesNotMatch(app, /disclosure\.addEventListener\('pointerenter'/);
  assert.doesNotMatch(app, /current\.addEventListener\('contextmenu'/);
  ...
  assert.match(css, /\.view-switcher-menu-grid\s*\{[\s\S]*grid-template-columns:\s*repeat\(2, minmax\(0, 1fr\)\)/);
  assert.match(css, /\.view-switcher-menu-item\.is-current::after/);
