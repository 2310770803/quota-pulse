...
test('settings destination labels stay compact in every locale', () => {
  const keys = [
  ...
  for (const locale of Object.keys(MESSAGES)) {
    for (const key of keys) {
      ...
      assert.ok(label, `${locale} should define ${key}`);
      assert.ok([...label].length <= 8, `${locale} ${key} should stay compact`);
    }
  }
});
