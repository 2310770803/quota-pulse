...
test('settings page uses a compact destination rail and one focused settings workspace', () => {
  const html = readRendererFile('index.html');
  assert.match(html, /class="settings-navigation"/);
  assert.match(html, /class="settings-workspace"/);
  assert.match(html, /id="settingsWorkspaceTitle"/);
  assert.match(html, /data-i18n="settings\.nav\.limits"/);
  assert.doesNotMatch(html, /class="settings-navigation-summary"/);
  assert.doesNotMatch(html, /class="settings-navigation-foot"/);
  ...
  assert.match(html, /class="settings-section-icon settings-section-icon-push"/);
  ...
  assert.match(html, /data-settings-target="general"/);
  assert.match(html, /data-settings-target="push"/);
  ...
  assert.match(app, /document\.querySelectorAll\('\[data-settings-target\]'\)/);
  ...
  assert.match(css, /\.settings-navigation-item\.is-active/);
  assert.match(css, /\.settings-workspace > \.settings-collapsible-group\.expanded/);
...
test('session archive retention has its own setting separate from Trends', () => {
  ...
  assert.match(main, /sessionUsageArchiveEnabled:\s*parseBoolean\(process\.env\.QUOTA_PULSE_SESSION_USAGE_ARCHIVE_ENABLED,\s*true\)/);
  ...
  assert.match(agent, /QUOTA_PULSE_SESSION_USAGE_ARCHIVE_ENABLED,\s*true\)/);
