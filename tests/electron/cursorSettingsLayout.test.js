...
test('OpenCode account panel provides multi-profile management', () => {
  ...
  assert.match(setupBody, /window\.quotaPulse\.openExternal\('https:\/\/opencode\.ai\/auth'\)/);
  assert.match(setupBody, /window\.quotaPulse\.opencode\.saveProfile\(/);
...
test('Codex account panel supports per-account enable toggles without showing timestamps', () => {
  ...
  assert.match(body, /window\.quotaPulse\.codex\.setAccountEnabled\(account\.id, input\.checked\)/);
...
test('Codex system account switching is exposed from limits account rows', () => {
  ...
  assert.match(renderHead, /window\.quotaPulse\.codex\.switchSystemAccount\(switchAccount\.id\)/);
  assert.match(renderHead, /state\.codexActiveAccount = result\.activeAccount/);
  assert.match(renderHead, /window\.quotaPulse\.codex\.refreshAccountLimits\(switchAccount\.id\)/);
...
test('DeepSeek account panel provides a first-class API key entry', () => {
  ...
  const app = readRendererFile('app.js');
  const setupBody = functionBodyBeforeMarker(app, 'setupCursorAccountUI', '\nsetupCursorAccountUI();');
  assert.match(setupBody, /window\.quotaPulse\.openExternal\('https:\/\/platform\.deepseek\.com\/api_keys'\)/);
...
test('Copilot account panel provides GitHub sign-in plus manual token fallback', () => {
  ...
  assert.match(setupBody, /window\.quotaPulse\.copilot\.signIn\(\{ flowId \}\)/);
  ...
  assert.match(setupBody, /window\.quotaPulse\.copilot\.cancelSignIn\(\{ flowId \}\)/);
...
test('Z.ai, Volcengine, Qoder, and Ollama account panels are exposed in settings', () => {
  ...
  assert.match(setupBody, /window\.quotaPulse\.openExternal\(zaiPlatformUrl\(\)\)/);
  assert.match(setupBody, /window\.quotaPulse\.openExternal\(volcenginePlatformUrl\(\)\)/);
  assert.match(setupBody, /window\.quotaPulse\.openExternal\(qoderPlatformUrl\(\)\)/);
  assert.match(setupBody, /ollamaCookie: input\.value/);
  assert.match(setupBody, /const validation = await window\.quotaPulse\.ollama\.validateCookie\(input\.value\);/);
  ...
  assert.match(setupBody, /window\.quotaPulse\.openExternal\(ollamaPlatformUrl\(\)\)/);
...
test('Kimi account panel stores web access separately and opens the allowlisted Code console', () => {
  ...
  assert.match(setupBody, /window\.quotaPulse\.openExternal\(kimiPlatformUrl\(\)\)/);
...
test('Claude Web account panel stores a redacted cookie and opens only the usage page', () => {
  ...
  assert.match(setupBody, /window\.quotaPulse\.claude\.saveCookie\(input\.value\)/);
  assert.match(setupBody, /if \(result\?\.superseded\) return;/);
  assert.ok(
    setupBody.indexOf('window.quotaPulse.claude.saveCookie(input.value)')
  ...
  assert.match(setupBody, /window\.quotaPulse\.openExternal\(claudePlatformUrl\(\)\)/);
...
test('MiMo account panel matches the manual Cookie provider layout', () => {
  ...
  assert.match(app, /window\.quotaPulse\.mimo\.openConsole\(\)/);
  assert.match(app, /window\.quotaPulse\.mimo\.addAccount\(input\.value\)/);
...
test('credential storage failures preserve the file and surface one actionable error', () => {
  ...
  assert.match(rendererSave, /state\.settings = await window\.quotaPulse\.getSettings\(\)/);
...
test('main settings normalize the Z.ai API region', () => {
  ...
  assert.match(defaults, /zaiApiRegion: normalizeZaiApiRegion\(process\.env\.QUOTA_PULSE_ZAI_API_REGION \|\| process\.env\.ZAI_API_REGION \|\| process\.env\.Z_AI_API_HOST \|\| 'global'\)/);
...
test('main settings normalize sync upload intervals and restart only the device runtime when it changes', () => {
  ...
  assert.match(envExample, /QUOTA_PULSE_SYNC_UPLOAD_INTERVAL_MS=0/);
  assert.match(envExample, /600000 \(10 min\).*1200000 \(20 min\).*1800000 \(30 min\)/);

  const defaults = main.slice(main.indexOf('function defaultSettings'), main.indexOf('function defaultLimitProviders'));
  assert.match(defaults, /syncUploadIntervalMs: normalizeSyncUploadIntervalMs\(process\.env\.QUOTA_PULSE_SYNC_UPLOAD_INTERVAL_MS\)/);
