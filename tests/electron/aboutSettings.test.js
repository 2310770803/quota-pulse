...
test('About uses runtime version and allowlisted Quota Pulse links', () => {
  const app = read('app.js');

  assert.match(app, /aboutVersion\.textContent = state\.appInfo\?\.version \? `v\$\{state\.appInfo\.version\}` : '—'/);
  assert.match(app, /QUOTA_PULSE_REPOSITORY_URL = 'https:\/\/github\.com\/Javis603\/quota-pulse'/);
  assert.match(app, /QUOTA_PULSE_ISSUES_URL = `\$\{QUOTA_PULSE_REPOSITORY_URL\}\/issues\/new\/choose`/);
  assert.match(app, /openRepositoryButton\?\.addEventListener\('click',[\s\S]*QUOTA_PULSE_REPOSITORY_URL/);
  assert.match(app, /reportIssueButton\?\.addEventListener\('click',[\s\S]*QUOTA_PULSE_ISSUES_URL/);
});
