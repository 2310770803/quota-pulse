...
test('Codex managed login selects and persists a workspace before account commit', () => {
  ...
  assert.match(app, /window\.quotaPulse\.codex\.selectWorkspace\(\{ flowId, workspaceId \}\)/);
});
...
test('Codex login renderer ignores stale flows and exposes explicit URL actions', () => {
  ...
  assert.match(setup, /window\.quotaPulse\.codex\.addAccount\(\{ flowId \}\)/);
  assert.match(setup, /window\.quotaPulse\.codex\.cancelLogin\(\{ flowId \}\)/);
  assert.match(setup, /const result = await window\.quotaPulse\.codex\.cancelLogin\(\{ flowId \}\);[\s\S]*if \(!result\?\.cancelled \|\| !isCurrentCodexSignInFlow\(flowId\)\) return;[\s\S]*state\.codexSignInFlowId = '';/);
  assert.match(setup, /isCurrentCodexSignInFlow\(status\.flowId\)/);
  assert.match(setup, /window\.quotaPulse\.openExternal\(state\.codexLoginUrl\)/);
