...
function tmpConfigHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'quota-pulse-autostart-'));
}

test('autostartSupported requires linux and a running AppImage', () => {
  assert.equal(autostartSupported({ platform: 'linux', env: { APPIMAGE: '/opt/Quota Pulse.AppImage' } }), true);
...
test('desktopFilePath honors XDG_CONFIG_HOME and falls back to ~/.config', () => {
  assert.equal(
    ...
    '/custom/config/autostart/quota-pulse.desktop'
  );
  assert.equal(
    desktopFilePath({ env: { HOME: '/home/frank' } }),
    '/home/frank/.config/autostart/quota-pulse.desktop'
  );
});

test('desktopFileContents produces a desktop entry pointing at the AppImage', () => {
  const contents = desktopFileContents('/opt/apps/Quota Pulse.AppImage');
  assert.match(contents, /^\[Desktop Entry\]\n/);
  assert.match(contents, /\nType=Application\n/);
  assert.match(contents, /\nName=Quota Pulse\n/);
  assert.match(contents, /\nExec="\/opt\/apps\/Quota Pulse\.AppImage"\n/);
...
test('desktopFileContents escapes literal percent signs in Exec arguments', () => {
  const contents = desktopFileContents('/opt/apps/Quota Pulse 100%.AppImage');
  const execLine = contents.split('\n').find((line) => line.startsWith('Exec='));
  assert.equal(execLine, 'Exec="/opt/apps/Quota Pulse 100%%.AppImage"');
});

test('setAutostartEnabled(true) writes the desktop file, creating the autostart dir', () => {
  const configHome = tmpConfigHome();
  const env = { XDG_CONFIG_HOME: configHome, APPIMAGE: '/opt/Quota Pulse.AppImage' };
  ...
  const written = fs.readFileSync(path.join(configHome, 'autostart', 'quota-pulse.desktop'), 'utf8');
  assert.match(written, /Exec="\/opt\/Quota Pulse\.AppImage"/);
});

test('isAutostartEnabled requires the desktop file to target the current AppImage', () => {
  const configHome = tmpConfigHome();
  const env = { XDG_CONFIG_HOME: configHome, APPIMAGE: '/opt/Quota Pulse.AppImage' };
  const staleEnv = { XDG_CONFIG_HOME: configHome, APPIMAGE: '/old/Quota Pulse.AppImage' };
