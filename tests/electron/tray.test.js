...
test('fallback tray icon source uses the current high-resolution AIQuotaPulse artwork', () => {
  const icon = fs.readFileSync(path.join(__dirname, '..', '..', '..', 'assets', 'app-icon.png'));
...
test('macOS tray icon downsamples the current full-color app artwork', () => {
  const calls = [];
  const resized = {};
  const image = {
  ...
  assert.match(calls[0][1], /assets[\\/]app-icon\.png$/);
...
test('non-macOS tray icon keeps the resized full-color app asset', () => {
  ...
  assert.match(calls[0][1], /assets[\\/]app-icon\.png$/);
  assert.deepEqual(calls.slice(1), [['resize', { width: 20, height: 20, quality: 'best' }]]);
});

test('renderer tray composition never falls back to Quota Pulse artwork', () => {
  const renderer = fs.readFileSync(path.join(__dirname, '..', '..', 'src', 'electron', 'renderer', 'app.js'), 'utf8');
  const main = fs.readFileSync(path.join(__dirname, '..', '..', 'src', 'electron', 'main.js'), 'utf8');
  assert.doesNotMatch(renderer, /tray-quota-pulse\.png/);
  assert.match(renderer, /state\.appInfo\?\.appIconDataUrl/);
  assert.match(main, /appIconDataUrl:\s*nativeImage[\s\S]*?createFromPath\(APP_ICON_PATH\)[\s\S]*?resize\(\{ width: 88, height: 88, quality: 'best' \}\)[\s\S]*?toDataURL\(\)/);
});
...
test('tray context menu complements the primary click with useful commands', () => {
  ...
  assert.deepEqual(template.map((item) => item.label || item.type), [
    'Refresh Now', 'Open View', 'separator', 'Tray Display', 'Window Presentation', 'separator', 'Version 0.27.0', 'Settings…', 'Quit Quota Pulse'
  ]);
...
test('tray context menu uses the selected locale for every visible level', () => {
  ...
  assert.deepEqual(template.map((item) => item.label || item.type), [
    '立即重新整理', '開啟頁面', 'separator', '托盤顯示', '視窗呈現方式', 'separator', '版本 0.27.0', '設定…', '結束 Quota Pulse'
  ]);
