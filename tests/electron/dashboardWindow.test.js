...
test('agent history collection defaults to enabled, matching the widget', () => {
  ...
  assert.match(agent, /QUOTA_PULSE_HISTORY_ENABLED,\s*true\)/);
  assert.doesNotMatch(envExample, /QUOTA_PULSE_HISTORY_ENABLED=0/);
  assert.match(configDoc, /QUOTA_PULSE_HISTORY_ENABLED=/);
});
...
test('dashboard.js fetches history over IPC and renders both tabs', () => {
  ...
  assert.match(js, /window\.quotaPulse\.getDashboardHistory\(\)/);
...
test('dashboard repains on a rate-only settings push, not just a currency-code change', () => {
  ...
  const js = read('src', 'electron', 'renderer', 'dashboard.js');
  const handler = /window\.quotaPulse\.onSettingsPush\?\.\(\(next\)\s*=>\s*\{([\s\S]*?)\n\}\);/.exec(js);
...
test('the trends preview opens the dashboard via IPC', () => {
  ...
  assert.match(app, /window\.quotaPulse\.openDashboard\(\)/);
});
