...
test('Limits and Home share reset expiry while preserving the existing reset copy', () => {
  ...
  assert.match(formatReset, /diffMs === 0\) return '立即重置'/);
  assert.match(formatReset, /return `\$\{formatDuration\(diffMs\)\}后重置`/);
...
test('limit percent tray mode renders provider icons into a generated tray image', () => {
  ...
  assert.match(maybeUpdateBarsIcon, /QuotaPulseTrayText\.isGeneratedTrayIconMode\(mode\)/);
...
test('Home uses explicit billing labels so Copilot Premium and Chat stay distinct', () => {
  ...
  assert.match(homeLabel, /if \(label\) return localizedLimitWindowLabel\(label\);/);
  assert.match(homeLabel, /billing: 'home\.limit\.billing'/);
  assert.match(homeLabel, /if \(window\?\.kind === 'balance'\) return '余额';/);
  ...
  assert.match(valueFormatter, /`\$\{formatMoney\(window\.amount, window\.currency\)\} 剩余`/);
...
test('settings provider status waits for stats and refreshes when stats arrive', () => {
  ...
  const statsPush = app.match(/window\.quotaPulse\.onStatsPush\?\.\(\(payload\) => \{[\s\S]*?\n\}\);/)?.[0] || '';
  const settingsPush = app.match(/window\.quotaPulse\.onSettingsPush\?\.\(\(next\) => \{[\s\S]*?\n\}\);/)?.[0] || '';
...
test('saving Ollama credentials enables its provider and always settles validation', () => {
  ...
  assert.match(ollamaSetup, /await window\.quotaPulse\.ollama\.validateCookie\(input\.value\)/);
